...
##########################################################################
def mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta, tau, nus, qid2lifeTime, roundIndex, qid2relativeLengthList):
        ...
        if len(Questions[targetQid]['answerList']) == 0: # current question hasn't got any answer, must write a new answer first
            ...
            if roundIndex in [1]:
                ...
                while (l <=0): # generate a l that > 0
                    l = np.random.normal(l_mean, l_std)
            elif roundIndex in [2]: # use the exact real answer length
...
def myFun(commName, commDir, rootDir, roundIndex, selected_reg_strengthList):
    ...
    logFileName = "semiSynthetic20_newModel_interaction_fixedTau_DataGenerating_Log.txt"
    ...
    # load real img and code info
    with open(intermediate_directory+'/'+'answerId2phId_img.dict', 'rb') as inputFile:
        answerId2phId_img = pickle.load( inputFile)
    with open(intermediate_directory+'/'+'answerId2phId_code.dict', 'rb') as inputFile:
    ...
    if commName != 'stackoverflow':
        ...
        qid2imgFlagList = defaultdict()
        qid2codeFlagList = defaultdict()
        ...
        for qid, d in ori_Questions.items():
            ...
            for ai,l in enumerate(d['lengthList']): # l is a list of tup (phid,len(text))
                ...
                if aid in answerId2phId_img.keys():
                    imgFlagList.append(1)
                else: # cur answer has no img
                    imgFlagList.append(0)
                if aid in answerId2phId_code.keys():
                    codeFlagList.append(1)
                else: # cur answer has no code
                    codeFlagList.append(0)
            assert (len(lengthList) == len(filtered_answerList) )
            qid2relativeLengthList[qid] = lengthList
            qid2imgFlagList[qid] = imgFlagList
            qid2codeFlagList[qid] = codeFlagList
    ...
    with open(intermediate_directory+'/'+f'semiSynthetic0_CVP_DataGenerating_round{roundIndex}_outputs.dict', 'wb') as outputFile:
        pickle.dump((qid2lifeTime,qid2answerCount,qid2relativeLengthList,qid2imgFlagList,qid2codeFlagList), outputFile)
    ...
    with open(intermediate_directory+'/'+'semiSynthetic0_CVP_DataGenerating_round2_outputs.dict', 'rb') as inputFile:
        qid2lifeTime,qid2answerCount,qid2relativeLengthList,qid2imgFlagList,qid2codeFlagList = pickle.load( inputFile)
    """
    # get prior alpha
    ...
    for reg_alpha in selected_reg_strengthList:
        ...
        try:
            ...
            qs = return_trainSuccess_dict_newModel[commName]['qs_sklearn']
        except:
        ...
        simulatedQuestions, generated_q_mean, generated_q_std, generated_neg_vote_count , generated_pos_vote_count, generated_answer_count, generated_qid2answerCount, generated_qid2lifetime = mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta, tau, nus, qid2lifeTime, roundIndex, qid2relativeLengthList)

        # save simulated Questions

        with open(intermediate_directory+'/'+f'simulated_data_bynewModel_interaction_fixedTau_round{roundIndex}_regAlpha({reg_alpha}).dict', 'wb') as outputFile:
        ...
        savePlot(fig, f"semiSynthetic20_dataGenerating_Histo_round{roundIndex}_regAlpha({reg_alpha}).png")

        # save csv statistics
        with open(rootDir+'/'+f'allComm_newModel_interaction_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( [commName,len(simulatedQuestions), generated_answer_count,reg_alpha,lamb,beta, generated_pos_vote_count, generated_neg_vote_count, generated_q_mean, generated_q_std, KLsynToOri_answerCount, KLsynToOri_eventCount])
...
def main():
    ...
    roundIndex = 1 ## multiple question multiple answer, original total event count, use real answer length fix tau =1, with RL,  q_std = 1, use lambda and beta (for different regularization strength) selected_reg_strengthList of each comm

    commName2selected_reg_strengthList = {'3dprinting.stackexchange':[20, 30,40,50,60, 70,80,90,100, 200, 300, 400, 500, 600, 700],
    ...
    with open(rootDir+'/'+f'allComm_newModel_interaction_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","total question Count", "generated Answer count", "reg_strength","prior_lambda","prior_beta","prior_delta","prior_epsilon","prior_zeta", "generated Pos Vote Count", "generated Neg Vote Count", "generated Quality mean", "generated Quality STD", "KL (generated Answer Count Per Question Distribution to prior)", "KL (generated Event Count Per Question Distribution to prior)"])
