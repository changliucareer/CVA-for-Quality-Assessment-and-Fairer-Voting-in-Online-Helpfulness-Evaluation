...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data_files, testingDataIndexListInFullData, log_file_name, reg_alpha, try_reg_strengthList, total_sample_count, original_n_feature, initial_batch_size, sampled_comms):
    ...
    if commName in sampled_comms:
        result_file_name = f"temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_model_file_name = f'temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_forSampledQuestion_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining11_trainCVP_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining11_trainCVP_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining11_trainCVP_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining11_trainCVP_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        plot_file_name = f'temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_forSampledQuestion_Losses.png'
    else:
        result_file_name = f"temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_results.txt"
        trained_model_file_name = f'temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining11_trainCVP_withSKLEARN_regAlpha({reg_alpha})_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining11_trainCVP_withSKLEARN_regAlpha({reg_alpha})_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining11_trainCVP_withlbfgs_regAlpha({reg_alpha})_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining11_trainCVP_withlbfgs_regAlpha({reg_alpha})_results.txt"
        plot_file_name = f'temperalOrderTraining11_trainCVP_regAlpha({reg_alpha})_Losses.png'
    log_text = ""
    ...
    if failFlagSklearn and failFlagLBFGS:
        ...
        # batch_size = 36866 # for debug of mathoverflow
        ...
        while (failToTrainSGD):
            try:
                ...
                for epoch in tqdm(range(int(max_iter_SGD)),desc='Training Epochs'):
                    # training #####################################################
                    model.train() 
    ...
    return total_sample_count, n_feature, coefs_sklearn, nus_sklearn, qs_sklearn, conformity_sklearn,coefs_lbfgs, nus_lbfgs, qs_lbfgs, conformity_lbfgs, CV_best_reg_alpha, CV_scores
...
def myFun(commIndex, commName, commDir, root_dir, splitted_comms,sampled_comms):
    ...
    log_file_name = "temperalOrderTraining11_CVP_Log.txt"
    ...
    resultFiles = [intermediate_directory+f"/temperalOrderTraining11_CVP_regAlpha(0.1)_forSampledQuestion_return.dict", intermediate_directory+f"/temperalOrderTraining11_CVP_regAlpha(0.1)_return.dict"]
    if os.path.exists(resultFiles[0]) or os.path.exists(resultFiles[1]):
    ...
    for reg_alpha in try_reg_strengthList:
        ...
        total_sample_count, n_feature, coefs_sklearn, nus_sklearn, qs_sklearn, conformity_sklearn,coefs_lbfgs, nus_lbfgs, qs_lbfgs, conformity_lbfgs, CV_best_reg_alpha, cur_CV_scores = output
        ...
        if (coefs_lbfgs != None) or (coefs_sklearn != None):
            ...
            if (commName).strip("sampled_") in sampled_comms:
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining11_CVP_regAlpha({reg_alpha})_forSampledQuestion_return.dict", 'wb') as outputFile:
                ...
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining11_CVP_regAlpha({reg_alpha})_return.dict", 'wb') as outputFile:
                    pickle.dump(return_trainSuccess_dict, outputFile)
        ...
        #     # save return dict
        #     with open(intermediate_directory+f"/temperalOrderTraining11_CVP_regAlpha({reg_alpha})_SGD_return.dict", 'wb') as outputFile:
        #         pickle.dump(return_trainSuccess_dict, outputFile)
        #         print( f"saved SGD return_trainSuccess_dict for {reg_alpha} of {commName}.")

        # # save csv
        with open(root_dir +'/'+'allComm_temperalOrderTraining11_CVP_results.csv', 'a', newline='') as csvfile:
            ...
            if (coefs_sklearn != None) and (coefs_lbfgs != None):
                writer.writerow( [commName,total_sample_count, reg_alpha, coefs_sklearn[0], q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, coefs_lbfgs[0], q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha])
            elif (coefs_sklearn != None):
                writer.writerow( [commName,total_sample_count, reg_alpha, coefs_sklearn[0], q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn,coefs_lbfgs, q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha])
        
        # # # save csv
        # with open(root_dir +'/'+'allComm_temperalOrderTraining11_CVP_SGD_results.csv', 'a', newline='') as csvfile:
        #     writer = csv.writer(csvfile, delimiter=',',
        #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
        #     if commName == 'stackoverflow':
        #         commName = 'reactjs_SOF'
        ...
        #         writer.writerow( [commName,total_sample_count, n_feature, len(testingDataIndexListInFullData), reg_alpha, 
        #                           coefs[0], 
        #                           q_mean, q_std, q_within1_percent, q_within2_percent,
        #                           nu_mean, nu_std, nu_within1_percent, nu_within2_percent, 
        #                           conformity, 
        #                           lowest_test_loss, test_accuracy, 
        #                           retrain_loss, retrain_accuracy, 
        #                           train_losses[lowest_test_loss_of_epoch-1], 
        #                           sklearn_accuracy, lowest_test_loss_of_epoch])
...
def main():
    ...
    # # # save csv
    # with open(root_dir +'/'+'allComm_temperalOrderTraining11_CVP_results_old.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #     writer.writerow( ["commName","totalSampleCount", "reg_strength", 
    #                       "coefs_sklearn", "q_sklearn_mean", "q_sklearn_std", "q_sklearn within (-1~1) percentage", "q_sklearn within (-2~2) percentage","nu_sklearn_mean", "nu_sklearn_std", "nu_sklearn within (-1~1) percentage", "nu_sklearn within (-2~2) percentage", "conformity_sklearn",
    #                       "coefs_lbfgs", "q_lbfgs_mean", "q_lbfgs_std", "q_lbfgs within (-1~1) percentage", "q_lbfgs within (-2~2) percentage","nu_lbfgs_mean", "nu_lbfgs_std", "nu_lbfgs within (-1~1) percentage", "nu_lbfgs within (-2~2) percentage", "conformity_lbfgs",
    #                       "CrossValidation_Score of current reg_alpha", "best reg_alpha by Cross Validation"])
    """
    # # save csv
    with open(root_dir +'/'+'allComm_temperalOrderTraining11_CVP_SGD_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","totalSampleCount","featureCount","testingDataSampleCount", "reg_strength", 
                          "coef_lambda_SGD", 
                          "q_SGD_mean", "q_SGD_std", "q_SGD within (-1~1) percentage", "q_SGD within (-2~2) percentage",
                          "nu_SGD_mean", "nu_SGD_std", "nu_SGD within (-1~1) percentage", "nu_SGD within (-2~2) percentage", 
                          "conformity_SGD",
                          "lowest_test_loss","test_accuracy",
                          "retrain_loss","retrain_accuracy",
                          "train_loss_of_lowest_test_loss_epoch",
                          "sklearn_accuracy","lowest_test_loss_epoch"
                         ])
        
    
    try:
        # test on comm "cstheory.stackexchange" to debug
        # myFun(1,commDir_sizes_sortedlist[256][0], commDir_sizes_sortedlist[256][1], root_dir)
        # test on comm "stackoverflow" to debug
        # myFun(2,commDir_sizes_sortedlist[359][0], commDir_sizes_sortedlist[359][1], root_dir)
        # test on comm "unix.meta.stackexchange" to debug
        myFun(3,commDir_sizes_sortedlist[173][0], commDir_sizes_sortedlist[173][1], root_dir)
        # # test on comm "politics.stackexchange" to debug
        # myFun(4,commDir_sizes_sortedlist[283][0], commDir_sizes_sortedlist[283][1], root_dir)
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining11 train CVP Done completely.    Elapsed: {:}.\n'.format(elapsed))
