...
import sklearn
from CustomizedNN import LRNN_1layer, LRNN_1layer_bias, LRNN_1layer_withoutRankTerm_specifyLambda_forNewModel
...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, roundIndex, variation, prior_lamb, prior_beta, reg_alpha, onlyRegQ):
    t0=time.time()
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha}){onlyRegQ_str}_training_results.txt"
    trained_model_file_name = f'semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha}){onlyRegQ_str}_model.sav'
    trained_withlbfgs_model_file_name = f'semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha}){onlyRegQ_str}_training_withlbfgs_model.sav'
    result_withlbfgs_file_name = f"semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha}){onlyRegQ_str}_training_withlbfgs_results.txt"
    plot_file_name = f'semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha}){onlyRegQ_str}_Losses.png'
    ...
    if modeltype == '1layer_withoutRankTerm':
        try:
            newWeight = [prior_lamb, prior_beta]
            model = LRNN_1layer_withoutRankTerm_specifyLambda_forNewModel(q_dim, newWeight) # initialize lambda and beta as prior
        except Exception as e:
    ...
    try:
        ...
        # training #####################################################
        model.train() 
        model.lamb.requires_grad = False  # don't update lambda
        if roundIndex in [6,7]:
        ...
        def loss_closure():
            ...
            for name, param in model.named_parameters():
                if onlyRegQ:
                    if name in ['lamb','beta']: # skip coefs, not regularize it 
                        continue
                    else:
        ...
        weights_lbfgs = list(parm['lamb'][0]) + list(parm['beta'][0]) + list(parm['qs'][0])
    ...
    ####################################################################################################
    print(f"start to train {commName} with cuda and torch SGD...")
    ...
    try:
        ...
        weights = list(parm['lamb'][0]) + list(parm['beta'][0]) + list(parm['qs'][0])
        ...
        return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, qs, conformity, coefs_lbfgs, bias_lbfgs, tau_lbfgs, qs_lbfgs, conformity_lbfgs
...
def myFun(commIndex, commName, commDir,root_dir, roundIndex, variation, try_reg_strengthList, onlyRegQ):
    ...
    log_file_name = "semiSynthetic15_newModelTraining_newTestingSplit_Log.txt"
    ...
    for reg_alpha in try_reg_strengthList:
        ...
        # resultFiles = [f'semiSynthetic15_newModel{variation}_round{roundIndex}_training_return.dict']
        ...
        # training 
        output = myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, roundIndex, variation, prior_lamb, prior_beta, reg_alpha, onlyRegQ)
        total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, qs, conformity, coefs_lbfgs, bias_lbfgs, tau_lbfgs, qs_lbfgs, conformity_lbfgs = output
        ...
        return_trainSuccess_dict[commName] = {'dataShape':(total_sample_count, n_feature), 'trainLosses':train_losses, 'testLosses':test_losses, 'epoch':lowest_test_loss_of_epoch, 'lowest_test_loss':lowest_test_loss, 'testAcc':test_accuracy,
                                            'coefs':coefs,'bias':bias,'tau':tau, 'qs':qs,
                                            'conformity':conformity,
                                            'coefs_lbfgs':coefs_lbfgs, 'bias_lbfgs':bias_lbfgs, 'qs_lbfgs':qs_lbfgs,
                                                'conformity_lbfgs':conformity_lbfgs}
        if onlyRegQ:
            # save return dict
            with open(intermediate_directory+f"/semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha})_onlyRegQ_training_return.dict", 'wb') as outputFile:
                pickle.dump(return_trainSuccess_dict, outputFile)
                print( f"saved return_trainSuccess_dict for regAlpha({reg_alpha}) of {commName}. tau:{tau},  conformity:{conformity}")

            # # save csv
            with open(root_dir +'/'+f'allComm_semiSynthetic15_newModel{variation}_round{roundIndex}_onlyRegQ_training_results.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',',
                                    quotechar='|', quoting=csv.QUOTE_MINIMAL)
                writer.writerow( [commName,total_sample_count,reg_alpha, coefs_lbfgs[0],coefs[0],coefs_lbfgs[1],coefs[1], qs_lbfgs[0],qs[0]])

        else:
            # save return dict
            with open(intermediate_directory+f"/semiSynthetic15_newModel{variation}_round{roundIndex}_regAlpha({reg_alpha})_training_return.dict", 'wb') as outputFile:
                pickle.dump(return_trainSuccess_dict, outputFile)
                print( f"saved return_trainSuccess_dict for regAlpha({reg_alpha}) of {commName}. tau:{tau},  conformity:{conformity}")

            # # save csv
            with open(root_dir +'/'+f'allComm_semiSynthetic15_newModel{variation}_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',',
                                    quotechar='|', quoting=csv.QUOTE_MINIMAL)
                writer.writerow( [commName,total_sample_count,reg_alpha, coefs_lbfgs[0],coefs[0],coefs_lbfgs[1],coefs[1], qs_lbfgs[0],qs[0]])
...
def main():
    ...
    if onlyRegQ:
        # # save csv
        with open(root_dir +'/'+f'allComm_semiSynthetic15_newModel{variation}_round{roundIndex}_onlyRegQ_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount","reg_strength", "lamb_newModel_LBFGS","lamb_newModel_SGD","beta_newModel_LBFGS", "beta_newModel_SGD", "q_newModel_LBFGS", "q_newModel_SGD"])

    else:
        # # save csv
        with open(root_dir +'/'+f'allComm_semiSynthetic15_newModel{variation}_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount","reg_strength", "lamb_newModel_LBFGS","lamb_newModel_SGD","beta_newModel_LBFGS", "beta_newModel_SGD", "q_newModel_LBFGS", "q_newModel_SGD"])
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('semiSynthetic15_newModelTraining_newTestingSplit Done completely.    Elapsed: {:}.\n'.format(elapsed))
