...
def myFun(commIndex, commName, commDir):
    ...
    log_file_name = "predictionAnalysis3_negLogLikelyhoods_CVP_forSplittedFiles_Log.txt"
    ...
    for i in range(3):
        ...
        trainingPercentageBatchFileDirList = [ f.path for f in os.scandir(predictionAnalysis_data_folder) if f.path.split('/')[-1].startswith(f"precent{trainingPercentage}PureData_batch_")]
        ...
        testingPercentageBatchFileDirList = [ f.path for f in os.scandir(predictionAnalysis_data_folder) if f.path.split('/')[-1].startswith(f"precent{testingPercentage}PureData_batch_")]
...
def main():
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('prediction analysis 3 NLL CVP training for splitted files Done completely.    Elapsed: {:}.\n'.format(elapsed))
