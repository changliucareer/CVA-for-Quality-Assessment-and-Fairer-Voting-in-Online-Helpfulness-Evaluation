...
import sklearn
from CustomizedNN import LRNN_1layer, LRNN_1layer_bias, LRNN_1layer_bias_specify,LRNN_1layer_bias_withoutRankTerm,LRNN_1layer_interaction,LRNN_1layer_interaction_bias
...
class MyIterableDataset(IterableDataset):
    def __init__(self, full_data,oneside, imgAndCode_data, testingDataIndexListInFullData, trainOrtest):
        self.data = full_data
        self.oneside = oneside
        self.imgAndCode_data = imgAndCode_data
        self.testingDataIndexListInFullData = testingDataIndexListInFullData
        self.trainOrtest = trainOrtest
    
    def process_data(self, dataset, oneside, imgAndCode_data, testingDataIndexListInFullData, trainOrtest):
        X = dataset[0].todense() # convert sparse matrix to np.array
        y = dataset[1]
        imgAndCode = imgAndCode_data.todense() # convert sparse matrix to np.array
        ...
        if oneside:
            X = np.concatenate((X[:,2:4], imgAndCode , X[:,4:] ), axis=1)  # for one side parametrization, removed the first two columns
        else: # two sides
            X = np.concatenate((X[:,:2] , X[:,3:4], imgAndCode,  X[:,4:]), axis=1) # for two sides parametrization, removed the third column
    ...
    def __iter__(self):
        return self.process_data(self.data, self.oneside, self.imgAndCode_data, self.testingDataIndexListInFullData, self.trainOrtest)
...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, imgAndCode_data, log_file_name):
    t0=time.time()

    #######################################################################
    ### training settings #######
    # result_file_name = f"temperalOrderTraining5_trainNewModel_posTau_interactionWithD_results.txt"
    # trained_model_file_name = f'temperalOrderTraining5_trainNewModel_posTau_interactionWithD_model.sav'
    # trained_withFullData_model_file_name = f'temperalOrderTraining5_trainNewModel_withFullData_posTau_interactionWithD_model.sav'
    # result_withFullData_file_name = f"temperalOrderTraining5_trainNewModel_withFullData_posTau_interactionWithD_results.txt"
    # plot_file_name = f'temperalOrderTraining5_trainNewModel_posTau_interactionWithD_Losses.png'
    # result_file_name = f"temperalOrderTraining5_trainNewModel_posTau_interactionWithReciprocalD_results.txt"
    # trained_model_file_name = f'temperalOrderTraining5_trainNewModel_posTau_interactionWithReciprocalD_model.sav'
    # trained_withFullData_model_file_name = f'temperalOrderTraining5_trainNewModel_withFullData_posTau_interactionWithReciprocalD_model.sav'
    # result_withFullData_file_name = f"temperalOrderTraining5_trainNewModel_withFullData_posTau_interactionWithReciprocalD_results.txt"
    # plot_file_name = f'temperalOrderTraining5_trainNewModel_posTau_interactionWithReciprocalD_Losses.png'
    # result_file_name = f"temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithD_results.txt"
    # trained_model_file_name = f'temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithD_model.sav'
    # trained_withFullData_model_file_name = f'temperalOrderTraining5_trainNewModel_withFullData_posTau_bias_interactionWithD_model.sav'
    # result_withFullData_file_name = f"temperalOrderTraining5_trainNewModel_withFullData_posTau_bias_interactionWithD_results.txt"
    # plot_file_name = f'temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithD_Losses.png'
    result_file_name = f"temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithReciprocalD_results.txt"
    trained_model_file_name = f'temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithReciprocalD_model.sav'
    trained_withFullData_model_file_name = f'temperalOrderTraining5_trainNewModel_withFullData_posTau_bias_interactionWithReciprocalD_model.sav'
    result_withFullData_file_name = f"temperalOrderTraining5_trainNewModel_withFullData_posTau_bias_interactionWithReciprocalD_results.txt"
    plot_file_name = f'temperalOrderTraining5_trainNewModel_posTau_bias_interactionWithReciprocalD_Losses.png'
    ...
    if oneside:
        n_feature = original_n_feature - 2 + 1 +2 # removed 2 columns for one side parametrization, and add 1 column of interaction column, and add 2 columns for img and code
    else:
        n_feature = original_n_feature - 1 + 1 +2 # removed 1 columns for two sides parametrization, and add 1 column of interaction column, and add 2 columns for img and code
    ...
    try:
        ...
        iterable_training_dataset = MyIterableDataset(full_data,oneside, imgAndCode_data, testingDataIndexListInFullData, 'train') 
        iterable_testing_dataset = MyIterableDataset(full_data,oneside, imgAndCode_data, testingDataIndexListInFullData, 'test') 
        iterable_full_dataset = MyIterableDataset(full_data,oneside, imgAndCode_data, testingDataIndexListInFullData, 'full') 
        # prepare data loader
        batch_size = 2000000
        ...
        print(f"start to retrain with full data for {commName} using epoch with lowest test loss {lowest_test_loss_of_epoch}...")
        ...
        for epoch in tqdm(range(lowest_test_loss_of_epoch),desc='Training Epochs'):
            # training #####################################################
            model.train() 
            batchIndex = 0
            for batch in my_full_dataloader:
                ...
                if epoch == lowest_test_loss_of_epoch-1:
        ...
        saveModel(model,trained_withFullData_model_file_name)
        print(f"for {commName} model saved.")
        ...
        text += f"      learnTau:{learnTau}, opt_forMiniBatchTrain :{opt_forMiniBatchTrain}, withBias:{withBias}, positiveTau:{positiveTau}, interactionType:{interactionType}\n"
        ...
        parm_withFullData = defaultdict()
        for name, param_withFullData in model.named_parameters():
            parm_withFullData[name]=param_withFullData.cpu().detach().numpy() 
        
        if withBias:
            bias_withFullData = parm_withFullData['linear.bias'][0]
        else:
            bias_withFullData = None
        
        if learnTau: 
            if positiveTau:
                tau_withFullData = np.exp(parm_withFullData['tau'].item())
            else:
                tau_withFullData = parm_withFullData['tau'].item()
        else:
            tau_withFullData = 1

        weights_withFullData = parm_withFullData['linear.weight'][0]
        ...
        return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, nus, qs, coefs_withFullData, bias_withFullData, tau_withFullData, nus_withFullData, qs_withFullData, conformity
...
def myFun(commIndex, commName, commDir):
    ...
    # log_file_name = "temperalOrderTraining5_newModel_interactionWithD_Log.txt"
    # log_file_name = "temperalOrderTraining5_newModel_interactionWithReciprocalD_Log.txt"
    # log_file_name = "temperalOrderTraining5_newModel_bias_interactionWithD_Log.txt"
    log_file_name = "temperalOrderTraining5_newModel_bias_interactionWithReciprocalD_Log.txt"
    ...
    # load img and code data
    try:
        with open(intermediate_directory+f"/imgAndCode_datasets_sorted_removeFirstRealVote.dict", 'rb') as inputFile:
            imgAndCode_data = pickle.load( inputFile)
    except:
        print(f"{commName} hasn't done img and code dataset yet, skip")
        return


    # load testing data (using the 76%~100% data as testing data)
    ...
    with open(predictionAnalysis_data_folder+f"/percent100Data.dict", 'rb') as inputFile:
        raw_testing_data = pickle.load( inputFile)
    sortingBaseListOfTestingData = [tup[0] for tup in raw_testing_data]
    ...
    # training 
    output = myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, imgAndCode_data,log_file_name)
    total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, nus, qs, coefs_withFullData, bias_withFullData, tau_withFullData, nus_withFullData, qs_withFullData, conformity = output
    ...
    # save return dict
    # with open(intermediate_directory+f"/temperalOrderTraining5_newModel_interactionWithD_return.dict", 'wb') as outputFile:
    # with open(intermediate_directory+f"/temperalOrderTraining5_newModel_interactionWithReciprocalD_return.dict", 'wb') as outputFile:
    # with open(intermediate_directory+f"/temperalOrderTraining5_newModel_bias_interactionWithD_return.dict", 'wb') as outputFile:
    with open(intermediate_directory+f"/temperalOrderTraining5_newModel_bias_interactionWithReciprocalD_return.dict", 'wb') as outputFile:
        pickle.dump(return_trainSuccess_dict, outputFile)
        print( f"saved return_trainSuccess_dict of {commName}. tau:{tau}, tau_withFullData:{tau_withFullData}, conformity:{conformity}, interactionTermCoef:{coefs[2]}, interactionTermCoef_withFullData:{coefs_withFullData[2]}, bias:{bias}, bias_withFullData:{bias_withFullData}")
...
def main():
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining5 train new Model with interaction term and imgAndcode terms  Done completely.    Elapsed: {:}.\n'.format(elapsed))
