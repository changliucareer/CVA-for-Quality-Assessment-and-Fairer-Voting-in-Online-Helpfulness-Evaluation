...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name):
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"temperalOrderTraining9_trainCVP_fixedTau_noRL_results.txt"
    trained_model_file_name = f'temperalOrderTraining9_trainCVP_fixedTau_noRL_model.sav'
    trained_withSKLEARN_model_file_name = f'temperalOrderTraining9_trainCVP_fixedTau_noRL_withSKLEARN_model.pkl'
    result_withSKLEARN_file_name = f"temperalOrderTraining9_trainCVP_fixedTau_noRL_withSKLEARN_results.txt"
    plot_file_name = f'temperalOrderTraining9_trainCVP_fixedTau_noRL_Losses.png'
    log_text = ""
    ...
    return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, conformity, coefs_sklearn, bias_sklearn, qs_sklearn, conformity_sklearn
...
def myFun(commIndex, commName, commDir, root_dir):
    ...
    log_file_name = "temperalOrderTraining9_CVP_fixedTau_noRL_Log.txt"
    ...
    total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, conformity, coefs_sklearn, bias_sklearn, qs_sklearn, conformity_sklearn = output
    ...
    if (coefs != None) or (coefs_sklearn != None):
        ...
        # save return dict
        with open(intermediate_directory+f"/temperalOrderTraining9_CVP_fixedTau_noRL_return.dict", 'wb') as outputFile:
            pickle.dump(return_trainSuccess_dict, outputFile)
            print( f"saved return_trainSuccess_dict of {commName}.")

    # # save csv
    with open(root_dir +'/'+'allComm_temperalOrderTraining9_CVP_fixedTau_noRL_results.csv', 'a', newline='') as csvfile:
...
def main():
    ...
    # # save csv
    with open(root_dir +'/'+'allComm_temperalOrderTraining9_CVP_fixedTau_noRL_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining9 train CVP with fixedTau=1 and without relative length terms Done completely.    Elapsed: {:}.\n'.format(elapsed))
