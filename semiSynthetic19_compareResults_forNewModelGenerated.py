...
def residulToDiagonalLine(x, y):
    ...
    return sum(squredResiduals), squredResiduals
...
def myPlot(commName, combineAll, plotFileName):
    ...
    Interaction = 'newModelInteractionGenerated' in plotFileName
    Quadratic = 'newModelQuadraticGenerated' in plotFileName

    plt.cla()
    fig, axs = plt.subplots(2, 2)
    fig.tight_layout(pad=3.0)

    # group_combineAll_basedOnPriorQAndVoteDiff = myGrouping(combineAll, sortingColumn1=0,sortingColumn2=1)
    
    # group_priorQ_rankZscores = [t[0] for t in group_combineAll_basedOnPriorQAndVoteDiff]
    # group_voteDiff_rankZscores = [t[1] for t in group_combineAll_basedOnPriorQAndVoteDiff]
    ...
    sumSquredResiduls_voteDiff, squredResiduals_voteDiff = residulToDiagonalLine(all_voteDiff_rankZscores, all_priorQ_rankZscores)
    axs[0,0].plot(all_voteDiff_rankZscores,p0(all_voteDiff_rankZscores),"g-", linewidth=1, 
                  label=f'slope={round(z0[0][0],4)}\nresidual={round(z0[1][0],4)}\nresidualToDiagonal={round(sumSquredResiduls_voteDiff,4)}')
    
    # # robust regression
    # Huber_robust_param_voteDiff,pred_y,Huber_robust_resid_voteDiff = robust_HuberT(all_voteDiff_rankZscores, all_priorQ_rankZscores)
    # axs[0,0].plot(all_voteDiff_rankZscores,pred_y,"g-", label=f'robust_Huber slope={round(Huber_robust_param_voteDiff,4)}\nresidual={round(Huber_robust_resid_voteDiff,4)}')
    # TheilSen_robust_param_voteDiff,pred_y,TheilSen_robust_resid_voteDiff = robust_TheilSen(all_voteDiff_rankZscores, all_priorQ_rankZscores)
    # axs[0,0].plot(all_voteDiff_rankZscores,pred_y,"y-", label=f'robust_TheilSen slope={round(TheilSen_robust_param_voteDiff,4)}\nresidual={round(TheilSen_robust_resid_voteDiff,4)}')
    # RANSAC_robust_param_voteDiff,pred_y,RANSAC_robust_resid_voteDiff, inlier_count = robust_RANSAC(all_voteDiff_rankZscores, all_priorQ_rankZscores)
    # inlier_ratio = inlier_count/len(all_voteDiff_rankZscores)
    # axs[0,0].plot(all_voteDiff_rankZscores,pred_y,"m-", label=f'robust_RANSAC slope={round(RANSAC_robust_param_voteDiff,4)}\nresidual={round(RANSAC_robust_resid_voteDiff,4)}\ninlierRatio={round(inlier_ratio,4)}')
    ...
    sumSquredResiduls_CVP, squredResiduals_CVP = residulToDiagonalLine(all_CVPsklearnQ_rankZscores, all_priorQ_rankZscores)
    axs[0,1].plot(all_CVPsklearnQ_rankZscores,p1(all_CVPsklearnQ_rankZscores),"r-", linewidth=1, 
                  label=f'slope={round(z1[0][0],4)}\nresidual={round(z1[1][0],4)}\nresidualToDiagonal={round(sumSquredResiduls_CVP,4)}')
    axs[0,1].plot(all_voteDiff_rankZscores,p0(all_voteDiff_rankZscores),"g-", linewidth=1)
    
    # # robust regression
    # Huber_robust_param_CVP,pred_y,Huber_robust_resid_CVP = robust_HuberT(all_CVPsklearnQ_rankZscores, all_priorQ_rankZscores)
    # axs[0,1].plot(all_CVPsklearnQ_rankZscores,pred_y,"g-", label=f'robust_Huber slope={round(Huber_robust_param_CVP,4)}\nresidual={round(Huber_robust_resid_CVP,4)}')
    # TheilSen_robust_param_CVP,pred_y,TheilSen_robust_resid_CVP = robust_TheilSen(all_CVPsklearnQ_rankZscores, all_priorQ_rankZscores)
    # axs[0,1].plot(all_CVPsklearnQ_rankZscores,pred_y,"y-", label=f'robust_TheilSen slope={round(TheilSen_robust_param_CVP,4)}\nresidual={round(TheilSen_robust_resid_CVP,4)}')
    # RANSAC_robust_param_CVP,pred_y,RANSAC_robust_resid_CVP, inlier_count = robust_RANSAC(all_CVPsklearnQ_rankZscores, all_priorQ_rankZscores)
    # inlier_ratio = inlier_count/len(all_CVPsklearnQ_rankZscores)
    # axs[0,1].plot(all_CVPsklearnQ_rankZscores,pred_y,"m-", label=f'robust_RANSAC slope={round(RANSAC_robust_param_CVP,4)}\nresidual={round(RANSAC_robust_resid_CVP,4)}\ninlierRatio={round(inlier_ratio,4)}')
   
    axs[0,1].legend(loc="best", fontsize = 6)

    # group_combineAll_basedOnPriorQAndNewModel = myGrouping(combineAll, sortingColumn1=0,sortingColumn2=5)
    # group_priorQ_rankZscores = [t[0] for t in group_combineAll_basedOnPriorQAndNewModel]
    # group_newModelsklearnQ_rankZscores = [t[5] for t in group_combineAll_basedOnPriorQAndNewModel]
    ...
    sumSquredResiduls_newModel, squredResiduals_newModel = residulToDiagonalLine(all_newModelsklearnQ_rankZscores, all_priorQ_rankZscores)
    axs[1,0].plot(all_newModelsklearnQ_rankZscores,p3(all_newModelsklearnQ_rankZscores),"r-", linewidth=1, 
                  label=f'slope={round(z3[0][0],4)}\nresidual={round(z3[1][0],4)}\nresidualToDiagonal={round(sumSquredResiduls_newModel,4)}')
    axs[1,0].plot(all_voteDiff_rankZscores,p0(all_voteDiff_rankZscores),"g-", linewidth=1)

    # # robust regression
    # Huber_robust_param_newModel,pred_y,Huber_robust_resid_newModel = robust_HuberT(all_newModelsklearnQ_rankZscores, all_priorQ_rankZscores)
    # axs[1,0].plot(all_newModelsklearnQ_rankZscores,pred_y,"g-", label=f'robust_Huber slope={round(Huber_robust_param_newModel,4)}\nresidual={round(Huber_robust_resid_newModel,4)}')
    # TheilSen_robust_param_newModel,pred_y,TheilSen_robust_resid_newModel = robust_TheilSen(all_newModelsklearnQ_rankZscores, all_priorQ_rankZscores)
    # axs[1,0].plot(all_newModelsklearnQ_rankZscores,pred_y,"y-", label=f'robust_TheilSen slope={round(TheilSen_robust_param_newModel,4)}\nresidual={round(TheilSen_robust_resid_newModel,4)}')
    # RANSAC_robust_param_newModel,pred_y,RANSAC_robust_resid_newModel, inlier_count = robust_RANSAC(all_newModelsklearnQ_rankZscores, all_priorQ_rankZscores)
    # inlier_ratio = inlier_count/len(all_newModelsklearnQ_rankZscores)
    # axs[1,0].plot(all_newModelsklearnQ_rankZscores,pred_y,"m-", label=f'robust_RANSAC slope={round(RANSAC_robust_param_newModel,4)}\nresidual={round(RANSAC_robust_resid_newModel,4)}\ninlierRatio={round(inlier_ratio,4)}')
    ...
    group_combineAll_basedOnAdvanced = myGrouping(combineAll, sortingColumn=7)
    group_priorQ_rankZscores = [t[0] for t in  group_combineAll_basedOnAdvanced]
    group_advancedsklearnQ_rankZscores = [t[7] for t in  group_combineAll_basedOnAdvanced]
    axs[1, 1].scatter(group_advancedsklearnQ_rankZscores, group_priorQ_rankZscores,s = 2)
    
    # ### using all raw points to fit
    all_priorQ_rankZscores = [t[0] for t in combineAll]
    all_advancedsklearnQ_rankZscores = [t[7] for t in combineAll]
    ...
    if Quadratic:
        axs[1, 1].set_xlabel('newModelQuadratic sklearn q rankZscore', fontsize = 8)
    else:
    ...
    # OLS fit
    z4,p4 = curveFitOLS(all_advancedsklearnQ_rankZscores, all_priorQ_rankZscores)
    sumSquredResiduls_advanced, squredResiduals_advanced = residulToDiagonalLine(all_advancedsklearnQ_rankZscores, all_priorQ_rankZscores)
    axs[1,1].plot(all_advancedsklearnQ_rankZscores,p4(all_advancedsklearnQ_rankZscores),"r-", linewidth=1, 
                  label=f'slope={round(z4[0][0],4)}\nresidual={round(z3[1][0],4)}\nresidualToDiagonal={round(sumSquredResiduls_advanced,4)}')
    ...
    if Interaction or Quadratic:
        ttest_voteDiff_residaul = scipy.stats.ttest_ind(squredResiduals_voteDiff, squredResiduals_advanced).statistic
        p_ofTtest_voteDiff_residaul = scipy.stats.ttest_ind(squredResiduals_voteDiff, squredResiduals_advanced).pvalue /2 # one-tailed
        ttest_CVP_residaul = scipy.stats.ttest_ind(squredResiduals_CVP, squredResiduals_advanced).statistic
        p_ofTtest_CVP_residaul = scipy.stats.ttest_ind(squredResiduals_CVP, squredResiduals_advanced).pvalue /2 # one-tailed
        ttest_newModel_residaul = scipy.stats.ttest_ind(squredResiduals_newModel, squredResiduals_advanced).statistic
        p_ofTtest_newModel_residaul = scipy.stats.ttest_ind(squredResiduals_newModel, squredResiduals_advanced).pvalue /2 # one-tailed
    else:
        ttest_voteDiff_residaul = scipy.stats.ttest_ind(squredResiduals_voteDiff, squredResiduals_newModel).statistic
        p_ofTtest_voteDiff_residaul = scipy.stats.ttest_ind(squredResiduals_voteDiff, squredResiduals_newModel).pvalue /2 # one-tailed
        ttest_CVP_residaul = scipy.stats.ttest_ind(squredResiduals_CVP, squredResiduals_newModel).statistic
        p_ofTtest_CVP_residaul = scipy.stats.ttest_ind(squredResiduals_CVP, squredResiduals_newModel).pvalue /2 # one-tailed
        ttest_newModel_residaul = None
        p_ofTtest_newModel_residaul = None

    fig.suptitle(f"{commName.replace('.stackexchange','')}\n({len(combineAll)})")

    savePlot(fig, plotFileName)

    tup = (z0,z1,z3,z4, sumSquredResiduls_voteDiff,sumSquredResiduls_CVP, sumSquredResiduls_newModel, sumSquredResiduls_advanced ,ttest_voteDiff_residaul, p_ofTtest_voteDiff_residaul, ttest_CVP_residaul, p_ofTtest_CVP_residaul, ttest_newModel_residaul, p_ofTtest_newModel_residaul,
...
def myFun(commName, commDir, rootDir, roundIndex, variation, reg_alpha1, reg_alpha2, try_reg_strengthList, sampled_comms, Interaction, Quadratic):
        ...
        if roundIndex in [1,2,3,4]:
            if Interaction: # new Model+Interaction generated
                try: # for sampled comms
                    with open(intermediate_directory+'/'+f"temperalOrderTraining13_newModel_interaction_fixedTau_regAlpha({reg_alpha1})_forSampledQuestion_return.dict", 'rb') as inputFile:
                        return_trainSuccess_dict_prior = pickle.load( inputFile)
                except:
                    with open(intermediate_directory+'/'+f"temperalOrderTraining13_newModel_interaction_fixedTau_regAlpha({reg_alpha1})_return.dict", 'rb') as inputFile:
                        return_trainSuccess_dict_prior = pickle.load( inputFile)
            elif Quadratic: # new Model+Quadratic generated
                try: # for sampled comms
                    with open(intermediate_directory+'/'+f"temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha1})_forSampledQuestion_return.dict", 'rb') as inputFile:
                        return_trainSuccess_dict_prior = pickle.load( inputFile)
                except:
                    with open(intermediate_directory+'/'+f"temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha1})_return.dict", 'rb') as inputFile:
                        return_trainSuccess_dict_prior = pickle.load( inputFile)
    ...
    print(f"return train success dict prior model loaded. length {len(return_trainSuccess_dict_prior)}")

    # learned coefs by CVP
    if roundIndex in [1,2,3,4]:
        if Interaction: # when new Model+Interatcion generated
            with open(intermediate_directory+'/'+f'semiSynthetic9_CVP{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_return.dict', 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_CVP = pickle.load( inputFile)
        elif Quadratic: # when new Model+Quadratic generated
            with open(intermediate_directory+'/'+f'semiSynthetic9_CVP{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_return.dict', 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_CVP = pickle.load( inputFile)
        else: # when new Model generated
    ...
    if roundIndex in [1,2,3,4]:
        if Interaction: # new Model+Interaction generated
            with open(intermediate_directory+'/'+f"semiSynthetic10_newModelTraining{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_newModel = pickle.load( inputFile)
        elif Quadratic: # new Model+Quadratic generated
            with open(intermediate_directory+'/'+f"semiSynthetic10_newModelTraining{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_newModel = pickle.load( inputFile)
        else: # new Model generated
    ...
    # learned coefs by advanced model (new model with interaction or Quadratic)
    if roundIndex in [1,2,3,4]:
        if Interaction: # new Model+Interaction generated
            with open(intermediate_directory+'/'+f"semiSynthetic21_newModelInteractionTraining{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_advanced = pickle.load( inputFile)
        elif Quadratic: # new Model+Quadratic generated
            with open(intermediate_directory+'/'+f"semiSynthetic26_newModelQuadraticTraining{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_advanced = pickle.load( inputFile)
        else: # new Model generated
            with open(intermediate_directory+'/'+f"semiSynthetic21_newModelInteractionTraining{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'rb') as inputFile:
                semiSynthetic_return_trainSuccess_dict_advanced = pickle.load( inputFile)
    print(f"semiSynthetic_return train success dict advanced model loaded. ")

    # get prior coefs
    try:
        if len(return_trainSuccess_dict_prior)==1:
            simplifiedCommName = list(return_trainSuccess_dict_prior.keys())[0]
        prior_coefs = return_trainSuccess_dict_prior[simplifiedCommName]['coefs_sklearn']
        prior_lamb = prior_coefs[0] # for one side training
        prior_beta = prior_coefs[1] # for one side training
        if Interaction or Quadratic:
            prior_delta = prior_coefs[2] # for one side training
        prior_nus = return_trainSuccess_dict_prior[simplifiedCommName]['nus_sklearn']
        prior_tau = 1
    except:
    ...
    # get learned coefs by training advanced model with SKLEARN
    try:
        semiSynthetic_coefs_advanced_sklearn = semiSynthetic_return_trainSuccess_dict_advanced[commName]['coefs_sklearn']
        semiSynthetic_lamb_advanced_sklearn = semiSynthetic_coefs_advanced_sklearn[0] # for one side training
        semiSynthetic_beta_advanced_sklearn = semiSynthetic_coefs_advanced_sklearn[1] # for one side training
        semiSynthetic_delta_advanced_sklearn = semiSynthetic_coefs_advanced_sklearn[2] # for one side training
        semiSynthetic_qs_advanced_sklearn = semiSynthetic_return_trainSuccess_dict_advanced[commName]['qs_sklearn']
        semiSynthetic_bias_advanced_sklearn = semiSynthetic_return_trainSuccess_dict_advanced[commName]['bias_sklearn']
        if semiSynthetic_bias_advanced_sklearn != None:
            semiSynthetic_qs_advanced_sklearn = [q + semiSynthetic_bias_advanced_sklearn for q in semiSynthetic_qs_advanced_sklearn] # update qs by adding bias

        semiSynthetic_nus_advanced_sklearn = semiSynthetic_return_trainSuccess_dict_advanced[commName]['nus_sklearn']
        semiSynthetic_tau_advanced_sklearn = 1
        semiSynthetic_CVscoresList_advanced = semiSynthetic_return_trainSuccess_dict_advanced[commName]['CV_scores'] # cross validation scores, a list of tup (KL, accuarcy), list length = length of try_reg_strengthList
        semiSynthetic_CVscores_advanced = semiSynthetic_CVscoresList_advanced[try_reg_strengthList.index(reg_alpha2)]
    except:
        print(f"No semiSynthetic new model training results for {commName}")
        return
    ...
    try:
        semiSynthetic_coefs_advanced = semiSynthetic_return_trainSuccess_dict_advanced[commName]['coefs_lbfgs']
        if semiSynthetic_coefs_advanced == None:
            semiSynthetic_lamb_advanced = None
            semiSynthetic_beta_advanced = None
            semiSynthetic_delta_advanced = None
        else:
            semiSynthetic_lamb_advanced = semiSynthetic_coefs_advanced[0] # for one side training
            semiSynthetic_beta_advanced = semiSynthetic_coefs_advanced[1] # for one side training
            semiSynthetic_delta_advanced = semiSynthetic_coefs_advanced[2] # for one side training
        semiSynthetic_qs_advanced = semiSynthetic_return_trainSuccess_dict_advanced[commName]['qs_lbfgs']
        semiSynthetic_bias_advanced = semiSynthetic_return_trainSuccess_dict_advanced[commName]['bias_lbfgs']
        if semiSynthetic_bias_advanced != None:
            semiSynthetic_qs_advanced = [q + semiSynthetic_bias_advanced for q in semiSynthetic_qs_advanced] # update qs by adding bias
        semiSynthetic_nus_advanced = semiSynthetic_return_trainSuccess_dict_advanced[commName]['nus_lbfgs']
        semiSynthetic_tau_advanced = 1
    except:
        print(f"No semiSynthetic new model training results for {commName}")
        return
    ...
    qid2advanced_nu = defaultdict()
    qid2advanced_sklearn_nu = defaultdict()
    ...
    for i, qid in enumerate(originalQidList):
        ...
        if semiSynthetic_nus_advanced != None:
            qid2advanced_nu[qid] = semiSynthetic_nus_advanced[i]
        else:
            qid2advanced_nu[qid] = 0
        qid2advanced_sklearn_nu[qid] = semiSynthetic_nus_advanced_sklearn[i]
    ...
    aid2advanced_q = defaultdict()
    aid2advanced_sklearn_q = defaultdict()
    for i, tup in enumerate(total_answersWithVotes_ids):
        ...
        if semiSynthetic_qs_advanced != None:
            q_advanced = semiSynthetic_qs_advanced[i]
        else:
            q_advanced = 0
        q_advanced_sklearn = semiSynthetic_qs_advanced_sklearn[i]
        ...
        aid2advanced_q[aid] = q_advanced
        aid2advanced_sklearn_q[aid] = q_advanced_sklearn
    ...
    if roundIndex in [1,2, 3,4]: 
        ...
        aid2advanced_q_rankZscore  = defaultdict()
        aid2advanced_sklearn_q_rankZscore  = defaultdict()
        ...
        aid2advanced_q_rank  = defaultdict()
        aid2advanced_sklearn_q_rank  = defaultdict()
        ...
        for qid, d in simulatedQuestions.items():
            ...
            # get ranks based on advanced_q
            involved_aid2advanced_q = {aid : aid2advanced_q[aid] for aid in involved_answerList}
            involved_aid2rankBasedOnAdvancedQ = defaultdict()
            for i, kv in enumerate(sorted(involved_aid2advanced_q.items(), key=lambda kv: (kv[1], kv[0]),reverse=True)) :
                aid = kv[0]
                involved_aid2rankBasedOnAdvancedQ[aid] = i+1 # rank
            
            aid2advanced_q_rank.update(involved_aid2rankBasedOnAdvancedQ)
            
            zScores = get_zScore(involved_aid2rankBasedOnAdvancedQ.values())
            for i,aid in enumerate(involved_aid2rankBasedOnAdvancedQ.keys()):
                aid2advanced_q_rankZscore[aid] = zScores[i]

            # get ranks based on advanced_q_sklearn
            involved_aid2advanced_sklearn_q = {aid : aid2advanced_sklearn_q[aid] for aid in involved_answerList}
            involved_aid2rankBasedOnAdvancedsklearnQ = defaultdict()
            for i, kv in enumerate(sorted(involved_aid2advanced_sklearn_q.items(), key=lambda kv: (kv[1], kv[0]),reverse=True)) :
                aid = kv[0]
                involved_aid2rankBasedOnAdvancedsklearnQ[aid] = i+1 # rank

            aid2advanced_sklearn_q_rank.update(involved_aid2rankBasedOnAdvancedsklearnQ)
            
            zScores = get_zScore(involved_aid2rankBasedOnAdvancedsklearnQ.values())
            for i,aid in enumerate(involved_aid2rankBasedOnAdvancedsklearnQ.keys()):
                aid2advanced_sklearn_q_rankZscore[aid] = zScores[i]

        # save results
        if Interaction:
            outputsFileName = f"semiSynthetic19_newModelInteractionGenerated{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_outputs.dict"
        elif Quadratic:
            outputsFileName = f"semiSynthetic19_newModelQuadraticGenerated{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_outputs.dict"
        else:
            outputsFileName = f"semiSynthetic19_newModelGenerated{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_outputs.dict"
        with open(intermediate_directory+f"/{outputsFileName}", 'wb') as outputFile:
            pickle.dump((aid2Prior_q_rankZscore,
                         ...
                         aid2advanced_q_rankZscore,
                         aid2advanced_sklearn_q_rankZscore,
                         aid2Prior_q_rank,
                         aid2VoteDiff_rank,
                         ...
                         aid2advanced_q_rank,
                         aid2advanced_sklearn_q_rank), outputFile)
            print( f"saved aid2zScore_outputs of {commName}")
        
        simulatedQuestions.clear() # to save memory
        ...
        for aid in aid2Prior_q_rankZscore.keys():
            tup = (aid2Prior_q_rankZscore[aid],
                   ...
                   aid2advanced_q_rankZscore[aid],
                   aid2advanced_sklearn_q_rankZscore[aid],
                   ...
                   aid2advanced_q_rank[aid],
                   aid2advanced_sklearn_q_rank[aid],
                   answer2parentQ[aid])
        ...
        for qid, aidList in qid2aidList.items():
            ...
            advancedqualityRanks = [aid2advanced_sklearn_q_rank[aid] for aid in aidList]
            ...
            kendalltauList_advanced.append(stats.kendalltau(priorQualityRanks, advancedqualityRanks).statistic)
        ...
        kendalltau_advanced = mean(kendalltauList_advanced)

        if Interaction or Quadratic:
            ttest_voteDiff = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_voteDiff).statistic
            p_ofTtest_voteDiff = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_voteDiff).pvalue /2 # one-tailed
            ttest_CVP = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_CVP).statistic
            p_ofTtest_CVP = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_CVP).pvalue /2 # one-tailed
            ttest_newModel = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_newModel).statistic
            p_ofTtest_newModel = scipy.stats.ttest_ind(kendalltauList_advanced, kendalltauList_newModel).pvalue /2 # one-tailed
        else:
            ttest_voteDiff = scipy.stats.ttest_ind(kendalltauList_newModel, kendalltauList_voteDiff).statistic
            p_ofTtest_voteDiff = scipy.stats.ttest_ind(kendalltauList_newModel, kendalltauList_voteDiff).pvalue /2 # one-tailed
            ttest_CVP = scipy.stats.ttest_ind(kendalltauList_newModel, kendalltauList_CVP).statistic
            p_ofTtest_CVP = scipy.stats.ttest_ind(kendalltauList_newModel, kendalltauList_CVP).pvalue /2 # one-tailed
            ttest_newModel = None
            p_ofTtest_newModel = None
        ...
        if Interaction or Quadratic:
            if Interaction:
                plotFileName = f"semiSynthetic19_newModelInteractionGenerated{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2}).pdf"
            elif Quadratic:
                plotFileName = f"semiSynthetic19_newModelQuadraticGenerated{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2}).pdf" # group by interval , 1layer grouping on x_axis
            tup = myPlot(commName, combineAll, plotFileName)
            z0,z1,z3,z4, sumSquredResiduls_voteDiff,sumSquredResiduls_CVP, sumSquredResiduls_newModel, sumSquredResiduls_advanced ,ttest_voteDiff_residaul, p_ofTtest_voteDiff_residaul, ttest_CVP_residaul, p_ofTtest_CVP_residaul, ttest_newModel_residaul, p_ofTtest_newModel_residaul = tup

        else:
            plotFileName = f"semiSynthetic6{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2}).pdf" # group by interval , 1layer grouping on x_axis
    ...
    if roundIndex in [1,2,3,4]:
        if Interaction or Quadratic:
            if Interaction:
                csvFileName = f'allComm_semiSynthetic_newModelInteraction{variation}_round{roundIndex}_resultComparison.csv'
            elif Quadratic:
                csvFileName = f'allComm_semiSynthetic_newModelQuadratic{variation}_round{roundIndex}_resultComparison.csv'
            
            with open(rootDir+'/'+f'{csvFileName}', 'a', newline='') as csvfile:
                ...
                writer.writerow( [commName,generated_event_count, reg_alpha1,reg_alpha2,
                                ...
                                prior_lamb, semiSynthetic_lamb_CVP_sklearn, semiSynthetic_lamb_CVP_torchLBFGS, semiSynthetic_lamb_newModel_sklearn, semiSynthetic_lamb_newModel, semiSynthetic_lamb_advanced_sklearn, semiSynthetic_lamb_advanced,
                                prior_beta, semiSynthetic_beta_CVP_sklearn, semiSynthetic_beta_CVP_torchLBFGS, semiSynthetic_beta_newModel_sklearn,semiSynthetic_beta_newModel, semiSynthetic_beta_advanced_sklearn,semiSynthetic_beta_advanced,
                                prior_delta, None, None, None,None, semiSynthetic_delta_advanced_sklearn,semiSynthetic_delta_advanced,
                                mean(aid2prior_q.values()), mean(aid2CVP_sklearn_q.values()), mean(aid2CVP_torchLBFGS_q.values()),mean(aid2newModel_sklearn_q.values()), mean(aid2newModel_q.values()),mean(aid2advanced_sklearn_q.values()), mean(aid2advanced_q.values()), 
                                mean(qid2prior_nu.values()), mean(qid2CVP_sklearn_nu.values()), mean(qid2CVP_torchLBFGS_nu.values()),mean(qid2newModel_sklearn_nu.values()),mean(qid2newModel_nu.values()),mean(qid2advanced_sklearn_nu.values()),mean(qid2advanced_nu.values()), 
                                ...
                                round(kendalltau_voteDiff,4),round(kendalltau_CVP,4), round(kendalltau_newModel,4), round(kendalltau_advanced,4),
                                ttest_voteDiff, p_ofTtest_voteDiff, ttest_CVP, p_ofTtest_CVP, ttest_newModel, p_ofTtest_newModel,
                                sumSquredResiduls_voteDiff, sumSquredResiduls_CVP, sumSquredResiduls_newModel, sumSquredResiduls_advanced, 
                                ttest_voteDiff_residaul, p_ofTtest_voteDiff_residaul, ttest_CVP_residaul, p_ofTtest_CVP_residaul, ttest_newModel_residaul, p_ofTtest_newModel_residaul,
                                ...
                                # TheilSen_robust_resid_voteDiff, TheilSen_robust_resid_CVP, TheilSen_robust_resid_newModel, TheilSen_robust_resid_newModelInteraction,
                                # RANSAC_robust_resid_voteDiff,RANSAC_robust_resid_CVP, RANSAC_robust_resid_newModel, RANSAC_robust_resid_newModelInteraction,
                                semiSynthetic_CVscores_CVP[0], semiSynthetic_CVscores_newModel[0], semiSynthetic_CVscores_advanced[0],
                                semiSynthetic_CVscores_CVP[1], semiSynthetic_CVscores_newModel[1], semiSynthetic_CVscores_advanced[1]])
            ...
            with open(rootDir+'/'+f'allComm_semiSynthetic_newModel{variation}_round{roundIndex}_resultComparison.csv', 'a', newline='') as csvfile:
                ...
                writer.writerow( [commName,generated_event_count, reg_alpha1,reg_alpha2,
                                ...
                                # round(KL_voteDiffToPriorQ_rankZscore,4),round(KL_CVPlearnedQToPriorQ_rankZscore,4), round(KL_newModellearnedQToPriorQ_rankZscore,4), round(KL_newModelInteractionlearnedQToPriorQ_rankZscore,4),
                                round(kendalltau_voteDiff,4),round(kendalltau_CVP,4), round(kendalltau_newModel,4), round(kendalltau_advanced,4),
                                ttest_voteDiff, p_ofTtest_voteDiff, ttest_CVP, p_ofTtest_CVP,
                                sumSquredResiduls_voteDiff, sumSquredResiduls_CVP, sumSquredResiduls_newModel, 
                                ttest_voteDiff_residaul, p_ofTtest_voteDiff_residaul, ttest_CVP_residaul, p_ofTtest_CVP_residaul,
                                avgSlopesForRankZscoreComparison[0],avgSlopesForRankZscoreComparison[1],avgSlopesForRankZscoreComparison[2],avgSlopesForRankZscoreComparison[3],
                                avgSlopesForRankZscoreComparison[4],avgSlopesForRankZscoreComparison[5],avgSlopesForRankZscoreComparison[6],avgSlopesForRankZscoreComparison[7],
                                avgSlopesForRankZscoreComparison[8],avgSlopesForRankZscoreComparison[9],avgSlopesForRankZscoreComparison[10],avgSlopesForRankZscoreComparison[11],
                                avgSlopesForRankComparison[0],avgSlopesForRankComparison[1],avgSlopesForRankComparison[2],avgSlopesForRankComparison[3],
...
def main():
    ...
    if Interaction or Quadratic: # when new Model+Interaction generaged
        if Interaction:
            csvFileName = f'allComm_semiSynthetic_newModelInteraction{variation}_round{roundIndex}_resultComparison.csv'
        elif Quadratic:
            csvFileName = f'allComm_semiSynthetic_newModelQuadratic{variation}_round{roundIndex}_resultComparison.csv'

        with open(rootDir+'/'+f'{csvFileName}', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( 
                            ["commName","total generated event Count", "reg_alpha1",'reg_alpha2',
                                ...
                                "prior_lamb", "learned_lamb_CVP_sklearn", "learned_lamb_CVP_torchLBFGS","learned_lamb_newModel_sklearn","learned_lamb_newModel_torchLBFGS","learned_lamb_advanced_sklearn","learned_lamb_advanced_torchLBFGS",
                                "prior_beta", "learned_beta_CVP_sklearn", "learned_beta_CVP_torchLBFGS","learned_beta_newModel_sklearn","learned_beta_newModel_torchLBFGS","learned_beta_advanced_sklearn","learned_beta_advanced_torchLBFGS",
                                "prior_delta", "learned_delta_CVP_sklearn", "learned_delta_CVP_torchLBFGS","learned_delta_newModel_sklearn","learned_delta_newModel_torchLBFGS","learned_delta_advanced_sklearn","learned_delta_advanced_torchLBFGS",
                                "prior_q_mean", "learned_q_CVP_sklearn_mean", "learned_q_CVP_torchLBFGS_mean","learned_q_newModel_sklearn_mean","learned_q_newModel_torchLBFGS_mean","learned_q_advanced_sklearn_mean","learned_q_advanced_torchLBFGS_mean",
                                "prior_nu_mean", "learned_nu_CVP_sklearn_mean", "learned_nu_CVP_torchLBFGS_mean","learned_nu_newModel_sklearn_mean","learned_nu_newModel_torchLBFGS_mean","learned_nu_advanced_sklearn_mean","learned_nu_advanced_torchLBFGS_mean", 
                                "slope_voteDiff", "slope_CVPsklearnQuality","slope_newModelsklearn_Quality","slope_advancedsklearn_Quality",
                                "residual_voteDiff", "residual_CVPsklearnQuality","residual_newModelsklearn_Quality","residual_advancedsklearn_Quality",
                                ...
                                "avgKT_voteDiffRankToPrior","avgKT_CVPqualityRankToPrior","avgKT_newModelqualityRankToPrior","avgKT_advancedqualityRankToPrior",
                                "ttest_voteDiff","p_ofTtest_voteDiff","ttest_CVP","p_ofTtest_CVP", "ttest_newModel","p_ofTtest_newModel",
                                "sumSquredResiduls_voteDiff","sumSquredResiduls_CVP","sumSquredResiduls_newModel","sumSquredResiduls_advanced",
                                "ttest_voteDiff_residaul","p_ofTtest_voteDiff_residaul","ttest_CVP_residaul","p_ofTtest_CVP_residaul","ttest_newModel_residaul","p_ofTtest_newModel_residaul",
                                "voteDiff_zscore_AvgSlope","CVP_zscore_AvgSlope","newModel_zscore_AvgSlope","advanced_zscore_AvgSlope",
                                "voteDiff_zscore_AvgResidual","CVP_zscore_AvgResidual","newModel_zscore_AvgResidual","advanced_zscore_AvgResidual",
                                "voteDiff_zscore_PositiveSlopePercentage","CVP_zscore_PositiveSlopePercentage","newModel_zscore_PositiveSlopePercentage","advanced_zscore_PositiveSlopePercentage",
                                "voteDiff_rank_AvgSlope","CVP_rank_AvgSlope","newModel_rank_AvgSlope","advanced_rank_AvgSlope",
                                "voteDiff_rank_AvgResidual","CVP_rank_AvgResidual","newModel_rank_AvgResidual","advanced_rank_AvgResidual",
                                "voteDiff_rank_PositiveSlopePercentage","CVP_rank_PositiveSlopePercentage","newModel_rank_PositiveSlopePercentage","advanced_rank_PositiveSlopePercentage",
                                ...
                                "crossValidation_KL_CVP","crossValidation_KL_newModel","crossValidation_KL_advanced",
                                "crossValidation_accuracy_CVP","crossValidation_accuracy_newModel","crossValidation_accuracy_advanced"])
    
    else: # when new Model generaged  
    ...
    for tup in selected_reg_strengthPairs:
        ...
        for comm in commDir_sizes_sortedlist:
            if comm[0] == commName:
                commDir = comm[1]
                break
        argsList.append((commName, commDir, rootDir, roundIndex, variation, reg_alpha1, reg_alpha2, try_reg_strengthList, sampled_comms, Interaction, Quadratic))
