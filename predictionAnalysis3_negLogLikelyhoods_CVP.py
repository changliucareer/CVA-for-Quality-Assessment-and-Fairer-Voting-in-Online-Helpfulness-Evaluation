...
class MyIterableDataset(IterableDataset):
    ...
    def process_data(self, dataset, oneside):
        ...
        # the first 5 columns of X are (pos_vote_ratio, neg_vote_ratio, seen_pos_vote_ratio, IPWrank, relative_length)
        if oneside:
...
def myTrain(commIndex, baseModelDir, trainingPercentage,trainingPercentageFileDir,testingPercentage,testingPercentageFileDir, commName, commDir, ori_questionCount, log_file_name):
    t0=time.time()
    # load data
    with open(trainingPercentageFileDir, 'rb') as inputFile:
        trainning_data = pickle.load( inputFile)
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"predictionAnalysis3_negLogLikelyhoods_CVP_trainWith{trainingPercentage}_testWith{testingPercentage}_results.txt"
    trained_model_file_name = f'predictionAnalysis3_negLogLikelyhoods_CVP_trainWith{trainingPercentage}_model.sav'
    plot_file_name = f'predictionAnalysis3_negLogLikelyhoods_CVP_trainWith{trainingPercentage}_testWith{testingPercentage}_Losses.png'
    ...
    total_sample_count = len(trainning_data[1])
    original_n_feature = trainning_data[0].todense().shape[1]
    ...
    try:
        ...
        iterable_training_dataset = MyIterableDataset(trainning_data,oneside)
...
def myFun(commIndex, commName, commDir):
    ...
    log_file_name = "predictionAnalysis3_negLogLikelyhoods_CVP_Log.txt"
    ...
    resultFiles = ['predictionAnalysis3_CVP_return.dict']
    ...
    for i in range(3):
        ...
        trainingPercentageFileDir = predictionAnalysis_data_folder+f"/percent{trainingPercentage}PureData.dict"
        testingPercentageFileDir = predictionAnalysis_data_folder+f"/percent{testingPercentage}PureData.dict"
        baseModelDir = None
        if i>0:
            baseModelDir = model_folder + f'/predictionAnalysis3_negLogLikelyhoods_CVP_trainWith{percentageList[i-1]}_model.sav'
        output = myTrain(commIndex, baseModelDir, trainingPercentage,trainingPercentageFileDir,testingPercentage,testingPercentageFileDir, commName, commDir, ori_questionCount, log_file_name)
    ...
    # save return dict
    with open(intermediate_directory+f"/predictionAnalysis3_CVP_return.dict", 'wb') as outputFile:
...
def main():
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('prediction analysis 3 NLL CVP  Done completely.    Elapsed: {:}.\n'.format(elapsed))
