...
def myFun(commName, commDir, return_trainSuccess_dict):
    ...
    try:
        ...
        # get total hard sampled answer count
        predictionAnalysis_hardSamples_folder = os.path.join(intermediate_directory, r'predictionAnalysis_hardSamples_folder')
        with open(predictionAnalysis_hardSamples_folder+f"/answersWithOnly5Votes.dict", 'rb') as inputFile:
            answersWithOnly5Votes_total = pickle.load( inputFile)
            total_annswerCountWithOnly5Votes = len(answersWithOnly5Votes_total)

        with open(intermediate_directory+f"/predictionAnalysis6_newModel_return.dict", 'rb') as inputFile:
            return_dict_newModel = pickle.load( inputFile)
        with open(intermediate_directory+f"/predictionAnalysis7_CVP_return.dict", 'rb') as inputFile:
            return_dict_CVP = pickle.load( inputFile)
        
        return_trainSuccess_dict[commName] = {'newModel':return_dict_newModel, 'CVP':return_dict_CVP, 'total_voteCount':total_voteCount, 'answerCountWith5Votes': total_annswerCountWithOnly5Votes}
...
def main():
    ...
    for commName, d in return_trainSuccess_dict.items():
        return_trainSuccess_normalDict[commName] = {'newModel':d['newModel'], 'CVP':d['CVP'], 'total_voteCount':d['total_voteCount'], 'answerCountWith5Votes':d['answerCountWith5Votes']}
    os.chdir(root_dir) # go back to root directory
    with open('predictionAnalysis_hardSamples.dict', 'wb') as outputFile:
    ...
    for commName, content in return_trainSuccess_dict.items():
        ...
        answerCountWith5Votes = content['answerCountWith5Votes']
        return_dict_newModel = content['newModel']
        return_dict_CVP = content['CVP']
        statisticsList.append((commName, total_voteCount, answerCountWith5Votes,
    ...
    with open('predictionAnalysis_hardSamples.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","total_voteCount", "answerCountWith5Votes",
    ...
    # Report progress.
    print('predictionAnalysis8_hardSamples_extractNegLogLikelyhoodsOfAllComm Done completely.    Elapsed: {:}.\n'.format(elapsed))
