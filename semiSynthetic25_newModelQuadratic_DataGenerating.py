...
##########################################################################
def mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta,delta, tau, qid2nu, qid2lifeTime, roundIndex, qid2relativeLengthList):
        ...
        if len(Questions[targetQid]['answerList']) == 0: # current question hasn't got any answer, must write a new answer first
            ...
            if roundIndex in [1]:
                ...
                while (l <=0): # generate a l that > 0
                    l = np.random.normal(l_mean, l_std)
            elif roundIndex in [2, 3, 4]: # use the exact real answer length
            ...
            except Exception as e: # when alpha is too small or negative
                print(e) 
            ...
            if toWrite == 1:
                ...
                r_of_curAnswer = ranksOfAnswersBeforeT[chosen_ai]
                cur_IPW = (1/(1+r_of_curAnswer))**tau
                ...
                z = q + lamb*seen_pvr + beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*cur_IPW
                if roundIndex in [3]: # doubled beta
                    z = q + lamb*seen_pvr + 2*beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*cur_IPW
                elif roundIndex in [4]: # tripled beta
                    z = q + lamb*seen_pvr + 3*beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*cur_IPW

                logit = 1.0 / (1 + np.exp(-z))
                
                try:
                    vote = bernoulli.rvs(logit, size=1)[0]
                except Exception as e: # when logit is too large or negative
                    print(e)
...
def myFun(commName, commDir, rootDir, roundIndex, selected_reg_strengthList,sampled_comms):

    print(f"comm {commName} running on {mp.current_process().name}")

    logFileName = "semiSynthetic25_newModelQuadratic_fixedTau_DataGenerating_Log.txt"
    ...
    for reg_alpha in selected_reg_strengthList:
        ...
        # load newModel one-side temperal training result
        if commName in sampled_comms:
            try:
                with open(intermediate_directory+'/'+f'temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_return.dict', 'rb') as inputFile:
                    return_trainSuccess_dict_newModel = pickle.load( inputFile)
            except:
                with open(intermediate_directory+'/'+f'temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_return.dict', 'rb') as inputFile:
                    return_trainSuccess_dict_newModel = pickle.load( inputFile)
        ...
        try:
            ...
            beta = coefs[1] # for one side training
            delta = coefs[2] # for one side training
            nus = return_trainSuccess_dict_newModel[simplifiedCommName]['nus_sklearn']
            qs = return_trainSuccess_dict_newModel[simplifiedCommName]['qs_sklearn']
        except:
            print(f"No newModel sklearn training results for {commName}")
            return
        ...
        if commName != 'stackoverflow':
            ...
            # # when using the whole stackoverflow 
            ...
            # when using subCommunity reactjs to represent stackoverflow
        ...
        assert len(nus) == ori_questionCount
        ...
        # save
        with open(intermediate_directory+'/'+f'semiSynthetic25_newModelQuadratic_DataGenerating_round{roundIndex}_outputs.dict', 'wb') as outputFile:
            pickle.dump((qid2lifeTime,qid2answerCount,qid2relativeLengthList), outputFile)
        print(f"semiSynthetic24_newModelInteraction_DataGenerating_outputs Saved for {commName}.")   
        """
        # load 
        ...
        simulatedQuestions, generated_q_mean, generated_q_std, generated_neg_vote_count , generated_pos_vote_count, generated_answer_count, generated_qid2answerCount, generated_qid2lifetime = mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta,delta, tau, qid2nu, qid2lifeTime, roundIndex, qid2relativeLengthList)

        # save simulated Questions

        with open(intermediate_directory+'/'+f'simulated_data_byNewModelQuadratic_fixedTau_round{roundIndex}_regAlpha({reg_alpha}).dict', 'wb') as outputFile:
        ...
        savePlot(fig, f"semiSynthetic25_dataGenerating_Histo_round{roundIndex}_regAlpha({reg_alpha}).png")

        # save csv statistics
        with open(rootDir+'/'+f'allComm_newModelQuadratic_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            if commName in sampled_comms:
                commNamePrint = 'sampled_'+commName
            else:
                commNamePrint = commName
            writer.writerow( [commNamePrint,len(simulatedQuestions), generated_answer_count,reg_alpha,lamb,beta, generated_pos_vote_count, generated_neg_vote_count, generated_q_mean, generated_q_std, KLsynToOri_answerCount, KLsynToOri_eventCount])
...
def main():
    ...
    with open(rootDir+'/'+f'allComm_newModelQuadratic_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","total question Count", "generated Answer count", "reg_strength","prior_lambda","prior_beta", "generated Pos Vote Count", "generated Neg Vote Count", "generated Quality mean", "generated Quality STD", "KL (generated Answer Count Per Question Distribution to prior)", "KL (generated Event Count Per Question Distribution to prior)"])
    ...
    # # test on comm "math.meta.stackexchange" to debug
    # myFun(commDir_sizes_sortedlist[250][0], commDir_sizes_sortedlist[250][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[250][0]])
    # # test on comm "mathoverflow.net" to debug
    # myFun(commDir_sizes_sortedlist[343][0], commDir_sizes_sortedlist[343][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[343][0]],sampled_comms)
     # # test on comm "askubuntu" to debug
    # myFun(commDir_sizes_sortedlist[356][0], commDir_sizes_sortedlist[356][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[356][0]],sampled_comms)
    # # test on comm "mathematica.stackexchange" to debug
    # myFun(commDir_sizes_sortedlist[331][0], commDir_sizes_sortedlist[331][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[331][0]],sampled_comms)
    # # test on comm philosophy.stackexchange" to debug
    # myFun(commDir_sizes_sortedlist[295][0], commDir_sizes_sortedlist[295][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[295][0]],sampled_comms)
    
     # test on comm codegolf.meta.stackexchange" to debug
    # myFun(commDir_sizes_sortedlist[221][0], commDir_sizes_sortedlist[221][1], rootDir, roundIndex, commName2selected_reg_strengthList[commDir_sizes_sortedlist[221][0]],sampled_comms)
    ...
    for tup in commDir_sizes_sortedlist:
        ...
        try:
            p = mp.Process(target=myFun, args=(commName,commDir, rootDir, roundIndex, selected_reg_strengthList,sampled_comms))
            p.start()
        except Exception as e:
            print(e)
            return
    ...
    # Report progress.
    print('semiSynthetic25_newModelQuadratic_DataGenerating Done completely.    Elapsed: {:}.\n'.format(elapsed))
