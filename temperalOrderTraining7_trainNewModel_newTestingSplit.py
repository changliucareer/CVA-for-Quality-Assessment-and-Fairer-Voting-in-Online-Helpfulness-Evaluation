...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name):
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"temperalOrderTraining7_trainNewModel_posTau_results.txt"
    trained_model_file_name = f'temperalOrderTraining7_trainNewModel_posTau_model.sav'
    plot_file_name = f'temperalOrderTraining7_trainNewModel_posTau_Losses.png'
    ...
    learning_rate = 0.1
    max_iter = 400   # this is the total number of epochs
    ############################################################################################
...
def myFun(commIndex, commName, commDir):
    ...
    total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, nus, qs, conformity = output
    ...
    # save return dict
    with open(intermediate_directory+f"/temperalOrderTraining7_newModel_return.dict", 'wb') as outputFile:
        pickle.dump(return_trainSuccess_dict, outputFile)
        print( f"saved return_trainSuccess_dict of {commName}. tau:{tau},  conformity:{conformity}")
...
def main():
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining7 train new Model with new testing splitting Done completely.    Elapsed: {:}.\n'.format(elapsed))
