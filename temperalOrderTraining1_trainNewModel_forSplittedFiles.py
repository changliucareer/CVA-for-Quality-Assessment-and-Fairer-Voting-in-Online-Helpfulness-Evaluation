...
class MyIterableDataset(IterableDataset):
    def __init__(self, batchFiles,oneside, testingDataIndexListInFullData, trainOrtest):
        self.dataDirs = batchFiles
        self.oneside = oneside
        self.testingDataIndexListInFullData = testingDataIndexListInFullData
        self.trainOrtest = trainOrtest
    
    def process_data(self, batchFileDirList, oneside, testingDataIndexListInFullData, trainOrtest):
        previousBatchSize = 0
        for b, subDir in enumerate(batchFileDirList):
            with open(subDir, 'rb') as inputFile:
                batchData= pickle.load( inputFile)
            X = batchData[0].todense() # convert sparse matrix to np.array
            y = batchData[1]
            ...
            for i in range(len(y)):
                fullIndex = previousBatchSize + i
                if trainOrtest == 'train': # True to train
                    if fullIndex not in testingDataIndexListInFullData:
                        yield X[i,:], y[i]
                elif trainOrtest == 'test': # False to test
                    if fullIndex in testingDataIndexListInFullData:
                        yield X[i,:], y[i]
                else: # need full data
                    yield X[i,:], y[i]
            
            previousBatchSize += len(y)
    
    def __iter__(self):
        return self.process_data(self.dataDirs, self.oneside, self.testingDataIndexListInFullData, self.trainOrtest)
...
def myTrain(commIndex, commName, commDir, ori_questionCount, batchFiles, testingDataIndexListInFullData, log_file_name):
    t0=time.time()

    #######################################################################
    ### training settings #######
    result_file_name = f"temperalOrderTraining1_trainNewModel_posTau_results.txt"
    trained_model_file_name = f'temperalOrderTraining1_trainNewModel_posTau_model.sav'
    trained_withFullData_model_file_name = f'temperalOrderTraining1_trainNewModel_withFullData_posTau_model.sav'
    result_withFullData_file_name = f"temperalOrderTraining1_trainNewModel_withFullData_posTau_results.txt"
    plot_file_name = f'temperalOrderTraining1_trainNewModel_posTau_Losses.png'
    ...
    for i, subDir in enumerate(batchFiles):
        batch = i+1
        print(f"scan batch {batch} of {commName} data ...")
        batchFileDir = subDir
        with open(batchFileDir, 'rb') as inputFile:
            batchData= pickle.load( inputFile)
            total_sample_count += batchData[0].shape[0]
    
    original_n_feature = batchData[0].todense().shape[1]
    batchData=tuple() # clear
    ...
    try:
        ...
        iterable_training_dataset = MyIterableDataset(batchFiles,oneside, testingDataIndexListInFullData, 'train') 
        iterable_testing_dataset = MyIterableDataset(batchFiles,oneside, testingDataIndexListInFullData, 'test') 
        iterable_full_dataset = MyIterableDataset(batchFiles,oneside, testingDataIndexListInFullData, 'full') 
        # prepare data loader
        batch_size = 10000
...
def myFun(commIndex, commName, commDir):
    ...
    log_file_name = "temperalOrderTraining1_newModel_Log.txt"
    ...
    resultFiles = ['temperalOrderTraining1_trainNewModel_withFullData_posTau_results.txt']
    ...
    sortedBatchesData_directory = os.path.join(splitted_intermediate_data_folder, r'Datasets_sorted_batches_folder')
    if not os.path.exists(sortedBatchesData_directory):
        print(f"{commName} has no Datasets_sorted_batches_folder.")
        return
    
    batchFiles = [ f.path for f in os.scandir(sortedBatchesData_directory) if f.path.split('/')[-1].startswith('whole_datasets_sorted_removeFirstRealVote_batch_')]
    # sort Files paths based on batch number
    batchFiles.sort(key=lambda p: int(p.strip(".dict").split("_")[-1]))
    batchesCount = len(batchFiles)
    print(f"found {batchesCount} batches of sorted data for {commName}")
    if batchesCount == 0:
        print(f"No sorted data for {commName}, skip")
        return
    ...
    predictionAnalysis_data_batchFiles = [ f.path for f in os.scandir(predictionAnalysis_data_folder) if f.path.split('/')[-1].startswith('percent100Data_batch_')]
    # sort Files paths based on batch number
    predictionAnalysis_data_batchFiles.sort(key=lambda p: int(p.strip(".dict").split("_")[-1]))
    for subDir in predictionAnalysis_data_batchFiles:
        with open(subDir, 'rb') as inputFile:
            batch_raw_testing_data = pickle.load( inputFile)
        sortingBaseListOfTestingData += [tup[0] for tup in batch_raw_testing_data]
        batch_raw_testing_data = [] # clear
    """
    ...
    print(f"extracting testingDataIndexListInFullData for {commName}")
    testingDataIndexListInFullData = [i for i, tup in enumerate(sorted_qidAndSampleIndex) if tup[3] in sortingBaseListOfTestingData]

    # save testingDataIndexListInFullData
    with open(intermediate_directory+f"/testingDataIndexListInFullData.dict", 'wb') as outputFile:
        pickle.dump(testingDataIndexListInFullData, outputFile)
        print(f"saved testingDataIndexListInFullData for {commName}")
    """
    # load back
    with open(intermediate_directory+f"/testingDataIndexListInFullData.dict", 'rb') as inputFile:
        testingDataIndexListInFullData = pickle.load( inputFile)
    

    # training 
    output = myTrain(commIndex, commName, commDir, ori_questionCount, batchFiles, testingDataIndexListInFullData, log_file_name)
    ...
    # save return dict
    with open(intermediate_directory+f"/temperalOrderTraining1_newModel_return.dict", 'wb') as outputFile:
        pickle.dump(return_trainSuccess_dict, outputFile)
        print( f"saved return_trainSuccess_dict of {commName}. tau:{tau}, tau_withFullData:{tau_withFullData}, conformity:{conformity}")
...
def main():
    ...
    splitted_comms = ['tex.stackexchange','meta.stackexchange','softwareengineering.stackexchange','superuser','math.stackexchange','worldbuilding.stackexchange']
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining1 train new Model   for splitted files Done completely.    Elapsed: {:}.\n'.format(elapsed))
