...
import sklearn
from CustomizedNN import LRNN_1layer_interaction, LRNN_1layer_interaction_bias, LRNN_1layer_interaction_withoutRankTerm, LRNN_1layer_interaction_bias_withoutRankTerm, LRNN_1layer_quadratic, LRNN_1layer_quadratic_bias, LRNN_1layer_quadratic_withoutRankTerm, LRNN_1layer_quadratic_bias_withoutRankTerm
    ...
    for j, coef in enumerate(weights):
        if not oneside: # when do twosides
            if j == 0: # the first feature is pos_vote_ratio. print lambda
                ...
                text += f"delta: {coef}\n" # the quadratic term of rank , print delta
                coefs.append(coef)
...
###################################################################################################
def myLogisticRegressionCV(Cs, solver,penalty, fit_intercept, max_iter, cv, commName, full_data, oneside, ori_questionCount, aid2prior_q, total_answersWithVotes_ids, qid2involvedAnswerList, learnTau, interactionType, tauColumnIndex):
    # split training and testing dataset
    kf = KFold(n_splits=cv, shuffle=True, random_state=1)

    myC2Scores = defaultdict()

    for ii, (train_indexList, test_indexList) in enumerate(kf.split(full_data[0].todense())):
        ...
        print(f"start cross validation training for {commName}, {ii+1}th split...")
        # training dataset
        batchIndex = 0
        for batch in my_train_dataloader:
            ...
            X = np.asarray(X)
            # add quadratic column
            quadratic_column = np.square(X[:,tauColumnIndex:tauColumnIndex+1])
                
            X = np.concatenate((X[:,:tauColumnIndex+1], quadratic_column, X[:,tauColumnIndex+1:]), axis=1)
            X = np.asmatrix(X)
            y = batch[1]
        ...
        for batch in my_test_dataloader:
            ...
            quadratic_column_test = np.square(X_test[:,tauColumnIndex:tauColumnIndex+1])
                
            X_test = np.concatenate((X_test[:,:tauColumnIndex+1], quadratic_column_test, X_test[:,tauColumnIndex+1:]), axis=1)
            X_test = np.asmatrix(X_test)
            y_test = batch[1]

        # train with different C
        for myC in Cs:
            print(f"train cross validation for {commName}, with myC={myC} for {ii+1}th split...")
            ...
            lr.fit(np.asarray(X), y)

            # test with testing data to get accuracy score
            cur_test_accuracy = lr.score(np.asarray(X_test), y_test)
            ...
            if learnTau: 
                print(f"sklearn can't learn tau!")
                tau = None
            else:
                tau= 1
            ...
            for j, tup in enumerate(total_answersWithVotes_ids):
                qid, aid = tup
                learned_q = qs[j]
...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data_files, testingDataIndexListInFullData, log_file_name, roundIndex, variation, reg_alpha1, reg_alpha2, try_reg_strengthList, CVPgenerated, aid2prior_q, total_answersWithVotes_ids, qid2involvedAnswerList, total_sample_count, original_n_feature,sampled_comms, Interaction, Quadratic):
    ...
    if Interaction: # new model+Interaction generated
        result_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
        trained_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_model.sav'
        trained_withSKLEARN_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
        result_withSKLEARN_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
        trained_withLBFGS_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
        result_withLBFGS_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
        plot_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_Losses.png'
    elif Quadratic: # new model+Quadratic generated
        result_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
        trained_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_model.sav'
        trained_withSKLEARN_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
        result_withSKLEARN_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
        trained_withLBFGS_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
        result_withLBFGS_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
        plot_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_Losses.png'
    
    elif CVPgenerated:
        result_file_name = f"semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
        trained_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_model.sav'
        trained_withSKLEARN_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
        result_withSKLEARN_file_name = f"semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
        trained_withLBFGS_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
        result_withLBFGS_file_name = f"semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
        plot_file_name = f'semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_Losses.png'
    else: # new model generated
        result_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
        trained_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_model.sav'
        trained_withSKLEARN_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
        result_withSKLEARN_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
        trained_withLBFGS_model_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
        result_withLBFGS_file_name = f"semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
        plot_file_name = f'semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_Losses.png'
    ...
    if CVPgenerated:
        if roundIndex in [21]:
            ...
            withBias = True # whether add bias term
        if roundIndex in [19, 1, 2, 21]:
            learnTau = False
            positiveTau = False
        else:
    ...
    if learnTau:
        if withBias:
            modeltype='1layer_bias_quadratic' 
        else: # without bias
            modeltype='1layer_quadratic'
    else:
        if withBias:
            modeltype='1layer_bias_quadratic_withoutRankTerm' 
        else: # without bias
            modeltype='1layer_quadratic_withoutRankTerm'
    
    opt_forMiniBatchTrain = 'sgd'
    learning_rate = 0.1
    max_iter = 2000   # this is the total number of epochs
    ############################################################################################
    ...
    if oneside:
        n_feature = original_n_feature - 2 + 1 # removed 2 columns for one side parametrization, and add 1 column of quadratic column
    else:
        n_feature = original_n_feature - 1 + 1 # removed 1 columns for two sides parametrization, and add 1 column of quadratic column
    ...
    try:
        ...
        for batch in my_full_dataloader:
            ...
            #     print(f"doing cross validation for {commName} reg pair ({reg_alpha1}, {reg_alpha2})...")
            #     try:
            #         myCs = [1/ (2*reg_alpha) for reg_alpha in try_reg_strengthList]
            #         if commName == 'stackoverflow': # the default _dtype for liblinear solver is [float64, float32]
            #             lrcv = LogisticRegressionCV(Cs= myCs, solver='liblinear',penalty='l2', fit_intercept=withBias, max_iter=max_iter, cv = 5)  # 5-fold
            #         else: # the default _dtype for lbfgs solver is float64
            ...
            #         print(f"fail to train cross validation for {commName} reg pair ({reg_alpha1}, {reg_alpha2}), {eee}")
            ...
            if reg_alpha2 == try_reg_strengthList[0]: # only do once CV for the first reg_alpha option
                ...
                # myCs = [1/ (2*reg_alpha) for reg_alpha in try_reg_strengthList]
                # myC2avgScores = myLogisticRegressionCV(Cs= myCs, solver='lbfgs',penalty='l2', fit_intercept=withBias, max_iter=max_iter, cv = 5, commName = commName, full_data=full_data, oneside=oneside, ori_questionCount=ori_questionCount, aid2prior_q = aid2prior_q, total_answersWithVotes_ids = total_answersWithVotes_ids, qid2involvedAnswerList= qid2involvedAnswerList, learnTau=learnTau, interactionType=interactionType, tauColumnIndex=tauColumnIndex)
            ...
            ##########################################################################################################
            print(f"training sklearn lr with current reg pair ({reg_alpha1}, {reg_alpha2}) for {commName}...")
            ...
            with open(final_directory+'/'+ trained_withSKLEARN_model_file_name,'wb') as f:
                pickle.dump(lr,f)
            print(f"for {commName} reg pair ({reg_alpha1}, {reg_alpha2}) model_withSKLEARN saved.")
            ...
            if learnTau: 
                if positiveTau:
                    tau_sklearn = np.exp(parm['tau'].item())
                else:
                    tau_sklearn = parm['tau'].item()
            else:
                tau_sklearn = 1

            weights_sklearn = lr.coef_[0]
            
            result_text_sklearn, coefs_sklearn,nus_sklearn, qs_sklearn = resultFormat(weights_sklearn, bias_sklearn, tau_sklearn, oneside, learnTau, ori_questionCount)
        ...
        log_text += f"fail to fit {commName} with sklearn LR. {e}\n"
        coefs_sklearn = None
        bias_sklearn = None
        nus_sklearn = None
        qs_sklearn = None
    ...
    if failFlagSklearn:
        ...
        try:
            ...
            if modeltype =='1layer_quadratic':
                model = LRNN_1layer_quadratic(input_dim,initial_tau,tauColumnIndex, positiveTau)
            elif modeltype =='1layer_bias_quadratic':
                model = LRNN_1layer_quadratic_bias(input_dim,initial_tau,tauColumnIndex, positiveTau)
            elif modeltype == '1layer_quadratic_withoutRankTerm':
                model = LRNN_1layer_quadratic_withoutRankTerm(input_dim,tauColumnIndex)
            elif modeltype == '1layer_bias_quadratic_withoutRankTerm':
                model = LRNN_1layer_quadratic_bias_withoutRankTerm(input_dim,tauColumnIndex)
            else:
            ...
            for batch in my_full_dataloader:
                ...
                print(f"for {commName} reg pair ({reg_alpha1}, {reg_alpha2}) model trained_withlbfgs saved.")
                ...
                result_text, coefs_lbfgs, nus_lbfgs, qs_lbfgs = resultFormat(weights_lbfgs, bias_lbfgs, tau_lbfgs, oneside, learnTau, ori_questionCount)
    ...
    try:
        ...
        return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, tau, nus, qs, conformity
    ...
    return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, nus, qs, tau, conformity,  coefs_lbfgs, bias_lbfgs, nus_lbfgs, qs_lbfgs, tau_lbfgs, conformity_lbfgs,  coefs_sklearn, bias_sklearn, nus_sklearn, qs_sklearn, tau_sklearn, conformity_sklearn, CV_best_reg_alpha, CV_scores
...
def myFun(commIndex, commName, commDir,root_dir, roundIndex, variation, selected_reg_strengthList, try_reg_strengthList, CVPgenerated,sampled_comms, Interaction, Quadratic):
    ...
    log_file_name = "semiSynthetic21_newModelInteractionTraining_newTestingSplit_Log.txt"
    ...
    for reg_alpha1 in selected_reg_strengthList:
        ...
        for reg_alpha2 in try_reg_strengthList:
            ...
            if Interaction: # new model+Interaction generated
                resultFiles = [f'semiSynthetic26_newModelQuadraticTraining{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict']
            elif Quadratic: # new model+Quadratic generated
                resultFiles = [f'semiSynthetic26_newModelQuadraticTraining{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict']
            elif CVPgenerated:
                resultFiles = [f'semiSynthetic26_newModelQuadraticTraining{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict']
            else: # new model generated
                resultFiles = [f'semiSynthetic26_newModelQuadraticTraining{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict']
            ...
            total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, nus, qs, tau, conformity,  coefs_lbfgs, bias_lbfgs, nus_lbfgs, qs_lbfgs, tau_lbfgs, conformity_lbfgs,  coefs_sklearn, bias_sklearn, nus_sklearn, qs_sklearn, tau_sklearn, conformity_sklearn, CV_best_reg_alpha, cur_CV_scores= output
            ...
            if (coefs_lbfgs != None) or (coefs_sklearn != None):
                ...
                return_trainSuccess_dict[commName] = {'dataShape':(total_sample_count, n_feature), 'trainLosses':train_losses, 'testLosses':test_losses, 'epoch':lowest_test_loss_of_epoch, 'lowest_test_loss':lowest_test_loss, 'testAcc':test_accuracy,
                                                    'coefs':coefs,'bias':bias,'tau':tau, 'nus':nus, 'qs':qs,'conformity':conformity,
                                                    'coefs_lbfgs':coefs_lbfgs,'bias_lbfgs':bias_lbfgs,'tau_lbfgs':tau_lbfgs, 'nus_lbfgs':nus_lbfgs, 'qs_lbfgs':qs_lbfgs,'conformity_lbfgs':conformity_lbfgs,
                                                    'coefs_sklearn':coefs_sklearn,'bias_sklearn':bias_sklearn,'tau_sklearn':tau_sklearn, 'nus_sklearn':nus_sklearn, 'qs_sklearn':qs_sklearn,'conformity_sklearn':conformity_sklearn,
                                                    'CV_best_reg_alpha':CV_best_reg_alpha, 'CV_scores':CV_scores}
                
                # save return dict
                if Interaction: # new model+Interaction generated
                    with open(intermediate_directory+f"/semiSynthetic26_newModelQuadraticTraining{variation}_newModelInteractionGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'wb') as outputFile:
                        pickle.dump(return_trainSuccess_dict, outputFile)
                elif Quadratic: # new model+Quadratic generated
                    with open(intermediate_directory+f"/semiSynthetic26_newModelQuadraticTraining{variation}_newModelQuadraticGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'wb') as outputFile:
                        pickle.dump(return_trainSuccess_dict, outputFile)
                elif CVPgenerated:
                    with open(intermediate_directory+f"/semiSynthetic26_newModelQuadraticTraining{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'wb') as outputFile:
                        pickle.dump(return_trainSuccess_dict, outputFile)
                else: # new model generated
                    with open(intermediate_directory+f"/semiSynthetic26_newModelQuadraticTraining{variation}_newModelGenerated_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_return.dict", 'wb') as outputFile:
                        pickle.dump(return_trainSuccess_dict, outputFile)
                    print( f"saved return_trainSuccess_dict of {commName}. tau_lbfgs:{tau_lbfgs},  conformity_lbfgs:{conformity_lbfgs}")

            # # save csv
            if Interaction: # new model+Interaction generated
                with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=',',
                                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    if (coefs_sklearn != None) and (coefs_lbfgs != None):
                        writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2,coefs_sklearn[0], coefs_lbfgs[0],coefs,coefs_sklearn[1],coefs_lbfgs[1],coefs, coefs_sklearn[2],coefs_lbfgs[2],coefs, mean(nus_sklearn), mean(nus_lbfgs),nus, mean(qs_sklearn), mean(qs_lbfgs),qs, tau_lbfgs, tau, CV_scores[try_reg_strengthList.index(reg_alpha2)][0],CV_scores[try_reg_strengthList.index(reg_alpha2)][1], CV_best_reg_alpha])
                    elif coefs_sklearn !=None:
                        writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2, coefs_sklearn[0], coefs_lbfgs,coefs, coefs_sklearn[1],coefs_lbfgs,coefs,coefs_sklearn[2],coefs_lbfgs,coefs, mean(nus_sklearn), nus_lbfgs,nus, mean(qs_sklearn), qs_lbfgs,qs, tau_lbfgs, tau, CV_scores[try_reg_strengthList.index(reg_alpha2)][0],CV_scores[try_reg_strengthList.index(reg_alpha2)][1], CV_best_reg_alpha])
            elif Quadratic: # new model+Quadratic generated
                with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=',',
                                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    if (coefs_sklearn != None) and (coefs_lbfgs != None):
                        writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2,coefs_sklearn[0], coefs_lbfgs[0],coefs,coefs_sklearn[1],coefs_lbfgs[1],coefs, coefs_sklearn[2],coefs_lbfgs[2],coefs, mean(nus_sklearn), mean(nus_lbfgs),nus, mean(qs_sklearn), mean(qs_lbfgs),qs, tau_lbfgs, tau, CV_scores[try_reg_strengthList.index(reg_alpha2)][0],CV_scores[try_reg_strengthList.index(reg_alpha2)][1], CV_best_reg_alpha])
                ...
                with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=',',
                                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    if (coefs_sklearn != None) and (coefs_lbfgs != None):
                        writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2,coefs_sklearn[0], coefs_lbfgs[0],coefs,coefs_sklearn[1],coefs_lbfgs[1],coefs, coefs_sklearn[2],coefs_lbfgs[2],coefs, mean(nus_sklearn), mean(nus_lbfgs),nus, mean(qs_sklearn), mean(qs_lbfgs),qs, tau_lbfgs, tau, CV_scores[try_reg_strengthList.index(reg_alpha2)][0],CV_scores[try_reg_strengthList.index(reg_alpha2)][1], CV_best_reg_alpha])
                ...
                with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=',',
                                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    if (coefs_sklearn != None) and (coefs_lbfgs != None):
                        writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2,coefs_sklearn[0], coefs_lbfgs[0],coefs,coefs_sklearn[1],coefs_lbfgs[1],coefs, coefs_sklearn[2],coefs_lbfgs[2],coefs, mean(nus_sklearn), mean(nus_lbfgs),nus, mean(qs_sklearn), mean(qs_lbfgs),qs, tau_lbfgs, tau, CV_scores[try_reg_strengthList.index(reg_alpha2)][0],CV_scores[try_reg_strengthList.index(reg_alpha2)][1], CV_best_reg_alpha])
...
def main():
    ...
    # # save csv
    if Interaction: # new model+Interaction generated
        with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelInteractionGenerated_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount", "reg_alpha1","reg_alpha2", "lambda_sklearn", "lambda_torchLBFGS","lambda_torchSGD","beta_sklearn","beta_LBFGS", "beta_SGD","delta_sklearn","delta_LBFGS", "delta_SGD", "nu_sklearn_mean", "nu_torchLBFGS_mean","nu_torchSGD_mean","q_sklearn_mean","q_torchLBFGS_mean","q_torchSGD_mean","tau_torchLBFGS", "tau_torchSGD", "cross validation learnedQ rankZscore to priorQ rankZscore average KL score", "cross validation voting prediction average accuracy score", "best cross validation reg_alpha"])
    elif Quadratic: # new model+Quadratic generated
        with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelQuadraticGenerated_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount", "reg_alpha1","reg_alpha2", "lambda_sklearn", "lambda_torchLBFGS","lambda_torchSGD","beta_sklearn","beta_LBFGS", "beta_SGD","delta_sklearn","delta_LBFGS", "delta_SGD", "nu_sklearn_mean", "nu_torchLBFGS_mean","nu_torchSGD_mean","q_sklearn_mean","q_torchLBFGS_mean","q_torchSGD_mean","tau_torchLBFGS", "tau_torchSGD", "cross validation learnedQ rankZscore to priorQ rankZscore average KL score", "cross validation voting prediction average accuracy score", "best cross validation reg_alpha"])

    elif CVPgenerated:
        with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount", "reg_alpha1","reg_alpha2", "lambda_sklearn", "lambda_torchLBFGS","lambda_torchSGD","beta_sklearn","beta_LBFGS", "beta_SGD","delta_sklearn","delta_LBFGS", "delta_SGD", "nu_sklearn_mean", "nu_torchLBFGS_mean","nu_torchSGD_mean","q_sklearn_mean","q_torchLBFGS_mean","q_torchSGD_mean","tau_torchLBFGS", "tau_torchSGD", "cross validation learnedQ rankZscore to priorQ rankZscore average KL score", "cross validation voting prediction average accuracy score", "best cross validation reg_alpha"])
    else: # new model generated
        with open(root_dir +'/'+f'allComm_semiSynthetic26_newModelQuadratic{variation}_newModelGenerated_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( ["commName","totalSampleCount", "reg_alpha1","reg_alpha2", "lambda_sklearn", "lambda_torchLBFGS","lambda_torchSGD","beta_sklearn","beta_LBFGS", "beta_SGD","delta_sklearn","delta_LBFGS", "delta_SGD", "nu_sklearn_mean", "nu_torchLBFGS_mean","nu_torchSGD_mean","q_sklearn_mean","q_torchLBFGS_mean","q_torchSGD_mean","tau_torchLBFGS", "tau_torchSGD", "cross validation learnedQ rankZscore to priorQ rankZscore average KL score", "cross validation voting prediction average accuracy score", "best cross validation reg_alpha"])
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('semiSynthetic26_newModelQuadraticTraining_newTestingSplit Done completely.    Elapsed: {:}.\n'.format(elapsed))
