    ...
    while( timeTick < n):
        ...
        if len(Questions[targetQid]['answerList']) == 0: # current question hasn't got any answer, must write a new answer first
            ...
            if toWrite == 1:
                ...
                z = q + lamb*seen_pvr + beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*seen_pvr
                if roundIndex in [3]: # doubled beta
                    z = q + lamb*seen_pvr + 2*beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*seen_pvr
                elif roundIndex in [4]: # tripled beta
                    z = q + lamb*seen_pvr + 3*beta*cur_IPW + qid2nu[targetQid]*rl + delta*cur_IPW*seen_pvr
...
def myFun(commName, commDir, rootDir, roundIndex, selected_reg_strengthList,sampled_comms):
    ...
    logFileName = "semiSynthetic24_newModelInteraction_fixedTau_DataGenerating_Log.txt"
    ...
    for reg_alpha in selected_reg_strengthList:
        ...
        # save
        with open(intermediate_directory+'/'+f'semiSynthetic24_newModelInteraction_DataGenerating_round{roundIndex}_outputs.dict', 'wb') as outputFile:
        ...
        with open(intermediate_directory+'/'+f'simulated_data_byNewModelInteraction_fixedTau_round{roundIndex}_regAlpha({reg_alpha}).dict', 'wb') as outputFile:
        ...
        savePlot(fig, f"semiSynthetic24_dataGenerating_Histo_round{roundIndex}_regAlpha({reg_alpha}).png")

        # save csv statistics
        with open(rootDir+'/'+f'allComm_newModelInteraction_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'a', newline='') as csvfile:
...
def main():
    ...
    with open(rootDir+'/'+f'allComm_newModelInteraction_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    # Report progress.
    print('semiSynthetic24_newModelInteraction_DataGenerating Done completely.    Elapsed: {:}.\n'.format(elapsed))
