...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, roundIndex, variation, reg_alpha1, reg_alpha2):
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
    trained_model_file_name = f'semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_model.sav'
    trained_withSKLEARN_model_file_name = f'semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
    result_withSKLEARN_file_name = f"semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
    trained_withLBFGS_model_file_name = f'semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
    result_withLBFGS_file_name = f"semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
    plot_file_name = f'semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_Losses.png'
    ...
    try:
        ...
        result_text, coefs_lbfgs, qs_lbfgs = resultFormat(weights_lbfgs, bias_lbfgs, tau_lbfgs, oneside, learnTau, ori_questionCount)
    ...
    return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, tau, conformity, coefs_lbfgs, bias_lbfgs, qs_lbfgs, tau_lbfgs, conformity_lbfgs
...
def myFun(commIndex, commName, commDir,root_dir, roundIndex, variation, selected_reg_strengthList, try_reg_strengthList):
    ...
    log_file_name = "semiSynthetic17_newModelTraining_newTestingSplit_Log.txt"
    ...
    # resultFiles = [f'semiSynthetic17_newModel{variation}_round{roundIndex}_training_return.dict']
    ...
    for reg_alpha1 in selected_reg_strengthList:
        ...
        for reg_alpha2 in try_reg_strengthList:
            ...
            total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, tau, conformity, coefs_lbfgs, bias_lbfgs, qs_lbfgs, tau_lbfgs, conformity_lbfgs = output
            ...
            return_trainSuccess_dict[commName] = {'dataShape':(total_sample_count, n_feature), 'trainLosses':train_losses, 'testLosses':test_losses, 'epoch':lowest_test_loss_of_epoch, 'lowest_test_loss':lowest_test_loss, 'testAcc':test_accuracy,
                                                'coefs':coefs,'bias':bias, 'qs':qs,'tau':tau, 'conformity':conformity,
                                                    'coefs_lbfgs':coefs_lbfgs, 'bias_lbfgs':bias_lbfgs, 'qs_lbfgs':qs_lbfgs,'tau_lbfgs':tau_lbfgs,
                                                    'conformity_lbfgs':conformity_lbfgs}
            
            # save return dict
            with open(intermediate_directory+f"/semiSynthetic17_newModel{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_return.dict", 'wb') as outputFile:
            ...
            # # save csv
            with open(root_dir +'/'+f'allComm_semiSynthetic17_newModel{variation}_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
                if coefs_lbfgs != None:
                    writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2,  coefs_lbfgs[0],coefs, mean(qs_lbfgs),qs, tau_lbfgs, tau])
...
def main():
    ...
    # # save csv
    with open(root_dir +'/'+f'allComm_semiSynthetic17_newModel{variation}_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","totalSampleCount", "reg_alpha1","reg_alpha2",  "coefs_torchLBFGS","coefs_torchSGD", "q_torchLBFGS_mean","q_torchSGD_mean", "tau_torchLBFGS","tau_torchSGD"])
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('semiSynthetic17_newModelTraining_newTestingSplit Done completely.    Elapsed: {:}.\n'.format(elapsed))
