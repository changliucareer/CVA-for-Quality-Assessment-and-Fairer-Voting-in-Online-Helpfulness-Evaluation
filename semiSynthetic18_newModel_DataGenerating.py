...
##########################################################################
def mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta, tau, qid2nu, qid2lifeTime, roundIndex, qid2relativeLengthList):
    ...
    while( timeTick < n):
        ...
        if len(Questions[targetQid]['answerList']) == 0: # current question hasn't got any answer, must write a new answer first
            ...
            if toWrite == 1:
                ...
                z = q + lamb*seen_pvr + beta*cur_IPW + qid2nu[targetQid]*rl
                if roundIndex in [3]: # doubled beta
                    z = q + lamb*seen_pvr + 2*beta*cur_IPW + qid2nu[targetQid]*rl
                elif roundIndex in [4]: # tripled beta
                    z = q + lamb*seen_pvr + 3*beta*cur_IPW + qid2nu[targetQid]*rl
...
def myFun(commName, commDir, rootDir, roundIndex, selected_reg_strengthList,sampled_comms):
    ...
    logFileName = "semiSynthetic18_newModel_fixedTau_DataGenerating_Log.txt"
    ...
    for reg_alpha in selected_reg_strengthList:
        ...
        # save
        with open(intermediate_directory+'/'+f'semiSynthetic18_newModel_DataGenerating_round{roundIndex}_outputs.dict', 'wb') as outputFile:
            pickle.dump((qid2lifeTime,qid2answerCount,qid2relativeLengthList), outputFile)
        print(f"semiSynthetic18_newModel_DataGenerating_outputs Saved for {commName}.")   
        """
        # load 
        ...
        simulatedQuestions, generated_q_mean, generated_q_std, generated_neg_vote_count , generated_pos_vote_count, generated_answer_count, generated_qid2answerCount, generated_qid2lifetime = mySimulation(commName, n, qid2theta, q_std, qid2RLmeanAndstd, lamb,beta, tau, qid2nu, qid2lifeTime, roundIndex, qid2relativeLengthList)

        # save simulated Questions

        with open(intermediate_directory+'/'+f'simulated_data_byNewModel_fixedTau_round{roundIndex}_regAlpha({reg_alpha}).dict', 'wb') as outputFile:
        ...
        savePlot(fig, f"semiSynthetic18_dataGenerating_Histo_round{roundIndex}_regAlpha({reg_alpha}).png")

        # save csv statistics
        with open(rootDir+'/'+f'allComm_newModel_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'a', newline='') as csvfile:
...
def main():
    ...
    # with open(rootDir+'/'+f'allComm_newModel_fixedTau_semiSyntheticDataGeneration_round{roundIndex}_statistics.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    """
    # selectedComms = ["coffee.stackexchange", "datascience.stackexchange", "webapps.stackexchange"]
