...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data_files, testingDataIndexListInFullData, log_file_name, reg_alpha, try_reg_strengthList, total_sample_count, original_n_feature, initial_batch_size,sampled_comms):
    ...
    if commName in sampled_comms:
        result_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        plot_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_Losses.png'
    else:
        result_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_results.txt"
        trained_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withSKLEARN_regAlpha({reg_alpha})_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withSKLEARN_regAlpha({reg_alpha})_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withlbfgs_regAlpha({reg_alpha})_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining20_trainNewModel_quadratic_fixedTau_withlbfgs_regAlpha({reg_alpha})_results.txt"
        plot_file_name = f'temperalOrderTraining20_trainNewModel_quadratic_fixedTau_regAlpha({reg_alpha})_Losses.png'
    log_text = ""
...
def myFun(commIndex, commName, commDir, root_dir, splitted_comms,sampled_comms):
    ...
    log_file_name = "temperalOrderTraining20_newModel_quadratic_Log.txt"
    ...
    for reg_alpha in try_reg_strengthList:
        ...
        resultFiles = [intermediate_directory+f"/temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_return.dict", intermediate_directory+f"/temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_return.dict"]
        ...
        if (coefs_lbfgs != None) or (coefs_sklearn != None):
            ...
            if (commName).strip("sampled_") in sampled_comms:
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_return.dict", 'wb') as outputFile:
                ...
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining20_newModel_quadratic_fixedTau_regAlpha({reg_alpha})_return.dict", 'wb') as outputFile:
                    pickle.dump(return_trainSuccess_dict, outputFile)
        ...
        # # save csv
        with open(root_dir +'/'+'allComm_temperalOrderTraining20_newModel_quadratic_fixedTau_results.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            if commName == 'stackoverflow':
                commNamePrint = 'reactjs_SOF'
            elif commName in sampled_comms:
                commNamePrint = 'sampled_' + commName.replace('.stackexchange','')
            else:
                commNamePrint = commName.replace('.stackexchange','')
            

            if (coefs_sklearn != None) and (coefs_lbfgs != None):
                writer.writerow( [commNamePrint,total_sample_count, reg_alpha, coefs_sklearn[0], coefs_sklearn[1], coefs_sklearn[2],  q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, coefs_lbfgs[0],coefs_lbfgs[1], coefs_lbfgs[2], q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha])
...
def main():
    ...
    # # # save csv
    # with open(root_dir +'/'+'allComm_temperalOrderTraining20_newModel_quadratic_fixedTau_results.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    selected_comms = ["cstheory.stackexchange", "stackoverflow", "politics.stackexchange","codegolf.meta.stackexchange",
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining20 train new model with quadratic rank term Done completely.    Elapsed: {:}.\n'.format(elapsed))
