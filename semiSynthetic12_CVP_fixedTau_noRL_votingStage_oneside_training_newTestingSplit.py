...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, roundIndex, variation, reg_alpha1, reg_alpha2, try_reg_strengthList):
    t0=time.time()

    #######################################################################
    ### training settings #######
    result_file_name = f"semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_results.txt"
    trained_model_file_name = f'semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_model.sav'
    trained_withSKLEARN_model_file_name = f'semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_model.pkl'
    result_withSKLEARN_file_name = f"semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withSKLEARN_results.txt"
    trained_withLBFGS_model_file_name = f'semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_model.sav'
    result_withLBFGS_file_name = f"semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_withLBFGS_results.txt"
    plot_file_name = f'semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_Losses.png'
    log_text = ""
    ...
    return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, conformity, coefs_sklearn, bias_sklearn, qs_sklearn, conformity_sklearn, coefs_lbfgs, bias_lbfgs, qs_lbfgs, conformity_lbfgs, CV_best_reg_alpha, CV_scores
...
def myFun(commIndex, commName, commDir, root_dir, roundIndex, variation, selected_reg_strengthList, try_reg_strengthList):
    ...
    log_file_name = f"semiSynthetic12_CVP{variation}_votingStage_oneside_training_newTestingSplit_Log.txt"
    ...
    # resultFiles = [f'semiSynthetic12_CVP{variation}_round{roundIndex}_training_return.dict']
    ...
    for reg_alpha1 in selected_reg_strengthList:
        ...
        for reg_alpha2 in try_reg_strengthList:
            output = myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, roundIndex, variation, reg_alpha1, reg_alpha2, try_reg_strengthList)
            total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias, qs, conformity, coefs_sklearn, bias_sklearn, qs_sklearn, conformity_sklearn, coefs_lbfgs, bias_lbfgs, qs_lbfgs, conformity_lbfgs, CV_best_reg_alpha, cur_CV_scores = output
            ...
            if (coefs != None) or (coefs_sklearn != None):
                ...
                # save return dict
                with open(intermediate_directory+f"/semiSynthetic12_CVP{variation}_round{roundIndex}_PriorRegAlpha({reg_alpha1})_regAlpha({reg_alpha2})_training_return.dict", 'wb') as outputFile:
            ...
            # # save csv
            with open(root_dir +'/'+f'allComm_semiSynthetic12_CVP{variation}_round{roundIndex}_training_results.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',',
                                    quotechar='|', quoting=csv.QUOTE_MINIMAL)
                if coefs != None:
                    writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2, coefs_sklearn[0], coefs_lbfgs[0],coefs[0], mean(qs_sklearn), mean(qs_lbfgs),mean(qs), CV_scores[try_reg_strengthList.index(reg_alpha2)], CV_best_reg_alpha])
                else:
                    writer.writerow( [commName,total_sample_count, reg_alpha1, reg_alpha2, coefs_sklearn[0], coefs_lbfgs[0],coefs, mean(qs_sklearn), mean(qs_lbfgs),qs, CV_scores[try_reg_strengthList.index(reg_alpha2)], CV_best_reg_alpha])
...
def main():
    ...
    # # save csv
    with open(root_dir +'/'+f'allComm_semiSynthetic12_CVP{variation}_round{roundIndex}_training_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","totalSampleCount","reg_alpha1","reg_alpha2", "lambda_sklearn", "lambda_torchLBFGS","lambda_torchSGD", "q_sklearn_mean", "q_torchLBFGS_mean","q_torchSGD_mean", "cross validation score", "best cross validation reg_alpha"])
    ...
    for tup in commDir_sizes_sortedlist:
        ...
        if commName not in selected_comms[:2]: # skip non selected communities
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('semiSynthetic12_CVP_fixedTau_noRL_votingStage_training  Done completely.    Elapsed: {:}.\n'.format(elapsed))
