...
def myTrain(commIndex, commName, commDir, ori_questionCount, batchFiles, testingDataIndexListInFullData, log_file_name):
    ...
    #######################################################################
    ### training settings #######
    result_file_name = f"temperalOrderTraining3_trainCVP_results.txt"
    trained_model_file_name = f'temperalOrderTraining3_trainCVP_model.sav'
    trained_withFullData_model_file_name = f'temperalOrderTraining3_trainCVP_withFullData_model.sav'
    result_withFullData_file_name = f"temperalOrderTraining3_trainCVP_withFullData_results.txt"
    plot_file_name = f'temperalOrderTraining3_trainCVP_Losses.png'
    ...
    try:
        ...
        result_text_withFullData, coefs_withFullData, nus_withFullData, qs_withFullData = resultFormat(weights_withFullData, bias_withFullData, oneside, ori_questionCount)
        ...
        return total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias,  nus, qs, coefs_withFullData, bias_withFullData,  nus_withFullData, qs_withFullData, conformity
...
def myFun(commIndex, commName, commDir):
    ...
    resultFiles = ['temperalOrderTraining3_trainCVP_withFullData_results.txt']
    ...
    try:
        ...
        print(f"{commName} hasn't done testingDataIndexListInFullData generation, doing now...")
        with open(intermediate_directory+f"/sorted_qidAndSampleIndexAndSortingBase.dict", 'rb') as inputFile:
            sorted_qidAndSampleIndex = pickle.load( inputFile)
    ...
    total_sample_count, n_feature, train_losses, test_losses, lowest_test_loss_of_epoch, lowest_test_loss, test_accuracy, coefs, bias,  nus, qs, coefs_withFullData, bias_withFullData, nus_withFullData, qs_withFullData, conformity = output
    ...
    # save return dict
    with open(intermediate_directory+f"/temperalOrderTraining3_CVP_return.dict", 'wb') as outputFile:
        pickle.dump(return_trainSuccess_dict, outputFile)
        print( f"saved return_trainSuccess_dict of {commName}. conformity:{conformity}")
...
def main():
    ...
    splitted_comms = ['tex.stackexchange','meta.stackexchange','softwareengineering.stackexchange','superuser','math.stackexchange','worldbuilding.stackexchange','codegolf.stackexchange']
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining3 train CVP  for splitted files Done completely.    Elapsed: {:}.\n'.format(elapsed))
