...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data_files, testingDataIndexListInFullData, log_file_name, reg_alpha, try_reg_strengthList, total_sample_count, original_n_feature, initial_batch_size, sampled_comms):
    ...
    if commName in sampled_comms:
        result_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_withSKLEARN_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_withlbfgs_regAlpha({reg_alpha})_forSampledQuestion_results.txt"
        plot_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_Losses.png'
    else:
        result_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_results.txt"
        trained_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_model.sav'
        trained_withSKLEARN_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_withSKLEARN_regAlpha({reg_alpha})_model.pkl'
        result_withSKLEARN_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_withSKLEARN_regAlpha({reg_alpha})_results.txt"
        trained_withlbfgs_model_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_withlbfgs_regAlpha({reg_alpha})_model.pkl'
        result_withlbfgs_file_name = f"temperalOrderTraining12_trainNewModel_fixedTau_withlbfgs_regAlpha({reg_alpha})_results.txt"
        plot_file_name = f'temperalOrderTraining12_trainNewModel_fixedTau_regAlpha({reg_alpha})_Losses.png'
    log_text = ""
    ...
    if failFlagSklearn:
        try:
            print(f"try to train {commName} using cuda and lbfgs...")
            # check gpu count
            ...
            print(f"for {commName} model trained_withlbfgs saved. avg training loss: {mean(trainingLosses)}")
            ...
            text += f"      max_iter = {max_iter},  learning_rate:{learning_rate}, batch_size:{total_sample_count}\n"
            text += f"dataset size: ({total_sample_count}, {n_feature})\n\n"
    ...
    if failFlagSklearn and failFlagLBFGS:
        print(f"try to train using cuda and SGD for {commName}...")
        # check gpu count
        cuda_count = torch.cuda.device_count()
        # assign one of gpu as device
        d = (commIndex) % cuda_count
        ...
        while (failToTrainSGD):
            try:
                ...
                text += f"      max_iter = {max_iter_SGD},  learning_rate:{learning_rate}\n"
                text += f"dataset size: ({total_sample_count}, {n_feature})\n testingData size: {len(testingDataIndexListInFullData)}\n"
                ...
                print(f"{commName} tried sgd with batch_size {batch_size}, failed: {ee}")
    ...
    return total_sample_count, n_feature, coefs_sklearn, nus_sklearn, qs_sklearn, conformity_sklearn,coefs_lbfgs, nus_lbfgs, qs_lbfgs, conformity_lbfgs,coefs, nus, qs, conformity, CV_best_reg_alpha, CV_scores, test_accuracy,lowest_test_loss,lowest_test_loss_of_epoch,batch_size
...
def myFun(commIndex, commName, commDir, root_dir, splitted_comms, sampled_comms):
    ...
    log_file_name = "temperalOrderTraining12_trainNewModel_Log.txt"
    ...
    if commName != 'stackoverflow':
        ...
        # # in case using whole SOF
        # ori_questionCount = 410516

    print(f"origianl questions count of {commName} is {ori_questionCount}.")
    ...
    if commName != 'stackoverflow':
        ...
        # # in case using the whole SOF (un-sorted data)
        # sortedBatchesData_directory = os.path.join(splitFolder_directory, r'Datasets_forEachQuestion_parts_folder')
        # full_data_files = [ f.path for f in os.scandir(sortedBatchesData_directory) if f.path.split('/')[-1].startswith("whole_datasets_sorted_removeFirstRealVote_batch_") ]
        # full_data_files.sort(key=lambda f: int(f.split('/')[-1].split('_')[-1].split('.')[0]))
    ...
    if commName in sampled_comms:
        ...
        if commName != 'stackoverflow':
            ...
            with open(subComm_intermediate_directory+f"/sorted_qidAndSampleIndexAndSortingBase.dict", 'rb') as inputFile:
                sorted_qidAndSampleIndex = pickle.load( inputFile)
        ...
        print(f"comm {commName} testingDataIndexListInFullData length: {len(testingDataIndexListInFullData)}")
    ...
    for reg_alpha in try_reg_strengthList:
        # if reg_alpha <=0.6: # already done for superuser
        #     initial_batch_size = 25125
        #     continue
        # # check whether already done this step, skip
        # resultFiles = [intermediate_directory+f"/temperalOrderTraining12_newModel_fixedTau_regAlpha({reg_alpha})_return.dict"]
        ...
        total_sample_count, n_feature, coefs_sklearn, nus_sklearn, qs_sklearn, conformity_sklearn,coefs_lbfgs, nus_lbfgs, qs_lbfgs, conformity_lbfgs,coefs, nus, qs, conformity, CV_best_reg_alpha, cur_CV_scores, test_accuracy,lowest_test_loss,lowest_test_loss_of_epoch,batch_size = output
        ...
        if qs_sklearn != None:
            ...
            q_sklearn_mean= None
            q_sklearn_std = None
            q_sklearn_within1_percent = None
            q_sklearn_within2_percent = None
        ...
        if nus_sklearn != None:
            ...
            nu_sklearn_mean= None
            nu_sklearn_std = None
            nu_sklearn_within1_percent = None
            nu_sklearn_within2_percent = None
        ...
        if (coefs_lbfgs != None) or ((coefs_sklearn != None) or (coefs != None)):
            return_trainSuccess_dict = defaultdict()
            return_trainSuccess_dict[commName] = {'dataShape':(total_sample_count, n_feature),
                                                 'epoch':lowest_test_loss_of_epoch, 'lowest_test_loss':lowest_test_loss, 'testAcc':test_accuracy,'mini_batch_size':batch_size,
            ...
            if (commName).strip("sampled_") in sampled_comms:
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining12_newModel_fixedTau_regAlpha({reg_alpha})_forSampledQuestion_return.dict", 'wb') as outputFile:
                ...
                # save return dict
                with open(intermediate_directory+f"/temperalOrderTraining12_newModel_fixedTau_regAlpha({reg_alpha})_return.dict", 'wb') as outputFile:
                    pickle.dump(return_trainSuccess_dict, outputFile)
        ...
        #     # save return dict
        #     with open(intermediate_directory+f"/temperalOrderTraining12_newModel_fixedTau_regAlpha({reg_alpha})_SGD_return.dict", 'wb') as outputFile:
        #         pickle.dump(return_trainSuccess_dict, outputFile)
        #         print( f"saved SGD return_trainSuccess_dict for {reg_alpha} of {commName}.")

        # # save csv
        with open(root_dir +'/'+'allComm_temperalOrderTraining12_newModel_fixedTau_results_new.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            if commName == 'stackoverflow':
                commName = 'reactjs_SOF'
                # commName = 'stackoverflow_whole'
            elif commName in sampled_comms:
            ...
            try:
                if (coefs_sklearn != None):
                    writer.writerow( [commName,total_sample_count, n_feature, len(testingDataIndexListInFullData), reg_alpha, "sklearn",
                                    coefs_sklearn[0], coefs_sklearn[1], q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, 
                                    CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha,
                                    coefs_lbfgs,coefs_lbfgs, q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, 
                                    coefs, coefs, q_mean, q_std, q_within1_percent, q_within2_percent,nu_mean, nu_std, nu_within1_percent, nu_within2_percent, conformity, 
                                    test_accuracy, lowest_test_loss, lowest_test_loss_of_epoch, batch_size])
                elif (coefs_lbfgs != None):
                    writer.writerow( [commName,total_sample_count, n_feature, len(testingDataIndexListInFullData), reg_alpha, "lbfgs",
                                    coefs_sklearn, coefs_sklearn, q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, 
                                    CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha,
                                    coefs_lbfgs[0], coefs_lbfgs[1], q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, 
                                    coefs, coefs, q_mean, q_std, q_within1_percent, q_within2_percent,nu_mean, nu_std, nu_within1_percent, nu_within2_percent, conformity, 
                                    test_accuracy, lowest_test_loss, lowest_test_loss_of_epoch, batch_size])
                elif (coefs != None): # write SGD results in sklearn columns
                    writer.writerow( [commName,total_sample_count, n_feature, len(testingDataIndexListInFullData), reg_alpha, "sgd",
                                    coefs_sklearn, coefs_sklearn, q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, 
                                    CV_scores[try_reg_strengthList.index(reg_alpha)] , CV_best_reg_alpha,
                                    coefs_lbfgs,coefs_lbfgs, q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, 
                                    coefs[0], coefs[1], q_mean, q_std, q_within1_percent, q_within2_percent,nu_mean, nu_std, nu_within1_percent, nu_within2_percent, conformity, 
                                    test_accuracy, lowest_test_loss, lowest_test_loss_of_epoch, batch_size])
            except Exception as eee:
                writer.writerow( [commName,total_sample_count, n_feature, len(testingDataIndexListInFullData), reg_alpha, "failed:"+str(eee),
                                    coefs_sklearn, coefs_sklearn, q_sklearn_mean, q_sklearn_std, q_sklearn_within1_percent, q_sklearn_within2_percent,nu_sklearn_mean, nu_sklearn_std, nu_sklearn_within1_percent, nu_sklearn_within2_percent, conformity_sklearn, 
                                    CV_scores , CV_best_reg_alpha,
                                    coefs_lbfgs, coefs_lbfgs, q_lbfgs_mean, q_lbfgs_std, q_lbfgs_within1_percent, q_lbfgs_within2_percent,nu_lbfgs_mean, nu_lbfgs_std, nu_lbfgs_within1_percent, nu_lbfgs_within2_percent, conformity_lbfgs, 
                                    coefs, coefs, q_mean, q_std, q_within1_percent, q_within2_percent,nu_mean, nu_std, nu_within1_percent, nu_within2_percent, conformity, 
                                    test_accuracy, lowest_test_loss, lowest_test_loss_of_epoch, batch_size])
        
        # # # save csv
        # with open(root_dir +'/'+'allComm_temperalOrderTraining12_newModel_fixedTau_SGD_results.csv', 'a', newline='') as csvfile:
        #     writer = csv.writer(csvfile, delimiter=',',
        #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
        #     if commName == 'stackoverflow':
        #         commName = 'reactjs_SOF'
...
def main():
    ...
    # # # save csv
    # with open(root_dir +'/'+'allComm_temperalOrderTraining12_newModel_fixedTau_results_new.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #     writer.writerow( ["commName","totalSampleCount","featureCount","testingDataSampleCount", "reg_strength", "trainType",
    #                       "coef_lambda_sklearn","coef_beta_sklearn", "q_sklearn_mean", "q_sklearn_std", "q_sklearn within (-1~1) percentage", "q_sklearn within (-2~2) percentage","nu_sklearn_mean", "nu_sklearn_std", "nu_sklearn within (-1~1) percentage", "nu_sklearn within (-2~2) percentage", "conformity_sklearn",
    #                       "CrossValidation_Score of current reg_alpha", "best reg_alpha by Cross Validation",
    #                       "coef_lambda_lbfgs","coef_beta_lbfgs", "q_lbfgs_mean", "q_lbfgs_std", "q_lbfgs within (-1~1) percentage", "q_lbfgs within (-2~2) percentage","nu_lbfgs_mean", "nu_lbfgs_std", "nu_lbfgs within (-1~1) percentage", "nu_lbfgs within (-2~2) percentage", "conformity_lbfgs",
    #                       "coef_lambda_SGD","coef_beta_SGD", "q_SGD_mean", "q_SGD_std", "q_SGD within (-1~1) percentage", "q_SGD within (-2~2) percentage","nu_SGD_mean", "nu_SGD_std", "nu_SGD within (-1~1) percentage", "nu_SGD within (-2~2) percentage", "conformity_SGD",
    #                       "test_accuracy", "lowest_test_loss", "lowest_test_loss_epoch", "mini_batch_size"])
    
    # # # save csv
    # with open(root_dir +'/'+'allComm_temperalOrderTraining12_newModel_fixedTau_SGD_results.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #     writer.writerow( ["commName","totalSampleCount","featureCount","testingDataSampleCount", "reg_strength", 
    #                       "coef_lambda_SGD","coef_beta_SGD", "q_SGD_mean", "q_SGD_std", "q_SGD within (-1~1) percentage", "q_SGD within (-2~2) percentage","nu_SGD_mean", "nu_SGD_std", "nu_SGD within (-1~1) percentage", "nu_SGD within (-2~2) percentage", "conformity_SGD","lowest_test_loss","test_accuracy","retrain_loss","retrain_accuracy","train_loss_of_lowest_test_loss_epoch","sklearn_accuracy","lowest_test_loss_epoch"
    #                      ])
    ...
    try:
        ...
        # myFun(4,commDir_sizes_sortedlist[173][0], commDir_sizes_sortedlist[173][1], root_dir, splitted_comms,sampled_comms)
        # # test on comm "cstheory.stackexchange" to debug
        # myFun(3,commDir_sizes_sortedlist[256][0], commDir_sizes_sortedlist[256][1], root_dir, splitted_comms,sampled_comms)
        # # # test on comm "politics.stackexchange" to debug
        # myFun(2,commDir_sizes_sortedlist[283][0], commDir_sizes_sortedlist[283][1], root_dir, splitted_comms,sampled_comms)
        # # test on comm "stackoverflow" to debug
        # myFun(1,commDir_sizes_sortedlist[359][0], commDir_sizes_sortedlist[359][1], root_dir, splitted_comms,sampled_comms)
        # test on comm "math.meta" to debug
        # myFun(1,commDir_sizes_sortedlist[250][0], commDir_sizes_sortedlist[250][1], root_dir, splitted_comms,sampled_comms)
        # test on comm "mathoverflow" to debug
        # myFun(2,commDir_sizes_sortedlist[343][0], commDir_sizes_sortedlist[343][1], root_dir, splitted_comms,sampled_comms)
        # test on comm "math" to debug
        # myFun(1,commDir_sizes_sortedlist[358][0], commDir_sizes_sortedlist[358][1], root_dir, splitted_comms,sampled_comms)
    except Exception as e:
        print(e)
        sys.exit()
    """
    ...
    # selected_comms = ['politics.stackexchange','stackoverflow','mathoverflow.net']
    # selected_comms = ['politics.stackexchange']

    # # for only try sklearn again
    # selected_comms = ['academia.stackexchange','askubuntu','ell.stackexchange',
    ...
    #                   'worldbuilding.stackexchange','physics.stackexchange','electronics.stackexhange',
    #                   'gis.stackexchange','codegolf.stackexchange','workplace.stackexchange']
    ...
    # selected_comms = ['mathoverflow.net','askubuntu']
    ...
    for i in range(360):
        tup = commDir_sizes_sortedlist[i]
        commName = tup[0]
        commDir = tup[1]

        # if commName not in sampled_comms: 
    ...
    elapsed = format_time(time.time() - t0)
    # Report progress.
    print('temperalOrderTraining12 train newModel Done completely.    Elapsed: {:}.\n'.format(elapsed))
