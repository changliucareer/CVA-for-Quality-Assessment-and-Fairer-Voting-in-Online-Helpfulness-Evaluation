...
def myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, reg_alpha):
    t0=time.time()
    ...
    C = 1/ (2*reg_alpha) # when using penalty='l2' and solver='lbfgs'

    # set LogisticRegression model 
    lr = LogisticRegression(solver='lbfgs',penalty='l2', fit_intercept=withBias, C=C, max_iter=max_iter)
    lrcv = LogisticRegressionCV(Cs= [0.00001,0.0001,0.001,0.01,0.1,1], solver='lbfgs',penalty='l2', fit_intercept=withBias, max_iter=max_iter)
...
def myFun(commIndex, commName, commDir, root_dir):
    ...
    for reg_alpha in try_reg_strengthList:
        output = myTrain(commIndex, commName, commDir, ori_questionCount, full_data, testingDataIndexListInFullData, log_file_name, reg_alpha)
        ...
        # # save csv
        with open(root_dir +'/'+'allComm_temperalOrderTraining10_CVP_fixedTau_noRL_results.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow( [commName,total_sample_count, reg_alpha, coefs_sklearn[0], coefs_lbfgs[0],coefs[0], q_sklearn_mean, q_sklearn_std, q_lbfgs_mean, q_lbfgs_std, q_mean, q_std])
...
def main():
    ...
    # # save csv
    with open(root_dir +'/'+'allComm_temperalOrderTraining10_CVP_fixedTau_noRL_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow( ["commName","totalSampleCount", "reg_strength", "coefs_sklearnLR", "coefs_torchLBFGS","coefs_torchSGD", "q_mean_sklearnLR", "q_std_sklearnLR", "q_mean_torchLBFGS", "q_std_torchLBFGS","q_mean_torchSGD","q_std_torchSGD"])
